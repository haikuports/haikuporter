...
class BuildPlatform(object):
    ...
    def init(self, treePath, outputDirectory, packagesPath, architecture, machineTriple):
    ...
    def getLicensesDirectory(self):
        ...
        if not directory:
            directory = self.findDirectory("B_SYSTEM_DIRECTORY") + "/data/licenses"
        return directory

    def getSystemMimeDbDirectory(self):
        directory = Configuration.getSystemMimeDbDirectory()
        if not directory:
            directory = self.findDirectory("B_SYSTEM_DIRECTORY") + "/data/mime_db"
        return directory
    ...
    def resolveDependencies(self, dependencyInfoFiles, requiresTypes, repositories, **kwargs):
        if not dependencyInfoFiles:
            return
        resolver = DependencyResolver(self, requiresTypes, repositories, **kwargs)
...
class BuildPlatformHaiku(BuildPlatform):
    ...
    def init(self, treePath, outputDirectory, packagesPath, shallowInitIsEnough=False):
        if not os.path.exists("/packages"):
            sysExit("haikuporter needs a version of Haiku with package management support")
        ...
        for entry in os.listdir(packagesDir):
            if entry == "haiku.hpkg" or (entry.startswith("haiku-") and entry.endswith(".hpkg")):
                systemPackageName = entry
                break
        if systemPackageName is None:
            sysExit("Failed to find Haiku system package")

        haikuPackageInfo = PackageInfo(os.path.join(packagesDir, systemPackageName))
        machine = MachineArchitecture.getTripleFor(haikuPackageInfo.architecture)
        if not machine:
            sysExit("Unsupported Haiku build platform architecture %s" % haikuPackageInfo.architecture)

        super(BuildPlatformHaiku, self).init(
            treePath, outputDirectory, packagesPath, haikuPackageInfo.architecture, machine
        )
    ...
    def findDirectory(self, which):
        ...
        if which not in self.findDirectoryCache:
            self.findDirectoryCache[which] = (
                check_output(["/bin/finddir", which]).decode("utf-8").rstrip()
            )  # drop newline
        return self.findDirectoryCache[which]

    def resolveDependencies(self, dependencyInfoFiles, requiresTypes, repositories, **kwargs):
        if not getOption("noSystemPackages"):
            systemPackagesDir = buildPlatform.findDirectory("B_SYSTEM_PACKAGES_DIRECTORY")
    ...
    def isSystemPackage(self, packagePath):
        return packagePath.startswith(self.findDirectory("B_SYSTEM_PACKAGES_DIRECTORY"))
    ...
    def activateBuildPackage(self, workDir, packagePath, revisionedName):
        ...
        self.deactivateBuildPackage(workDir, activeBuildPackage, revisionedName)
    ...
    def deactivateBuildPackage(self, workDir, activeBuildPackage, revisionedName):
    ...
    def setupNonChrootBuildEnvironment(self, workDir, secondaryArchitecture, requiredPackages):
        sysExit("setupNonChrootBuildEnvironment() not supported on Haiku")

    def cleanNonChrootBuildEnvironment(self, workDir, secondaryArchitecture, buildOK):
    ...
    def _waitForPackageSelfLink(self, revisionedName, activated):
        while True:
            try:
                time.sleep(0.5)
                linkTarget = os.readlink("/packages/%s/.self" % revisionedName)
                packagingFolder = revisionedName.split("-")[0]
                linkTargetIsPackagingFolder = os.path.basename(linkTarget) == packagingFolder
                ...
                if not activated:
                    return
            info(
                "waiting for build package %s to be %s" % (revisionedName, "activated" if activated else "deactivated")
            )
            time.sleep(1)
...
class BuildPlatformUnix(BuildPlatform):
    ...
    def init(self, treePath, outputDirectory, packagesPath, shallowInitIsEnough=False):
        ...
        super(BuildPlatformUnix, self).init(treePath, outputDirectory, packagesPath, architecture, machine)

        self.secondaryTargetArchitectures = Configuration.getSecondaryTargetArchitectures()

        if not shallowInitIsEnough:
            if Configuration.getPackageCommand() is None:
                sysExit("--command-package must be specified on this build platform!")
            if Configuration.getMimesetCommand() == "mimeset":
                sysExit("--command-mimeset must be specified on this build platform!")
            ...
            if self.secondaryTargetArchitectures:
                if not Configuration.getSecondaryCrossToolsDirectory(self.secondaryTargetArchitectures[0]):
                    sysExit(
                        "The cross-tools directories for all secondary "
                        "architectures must be specified on this build "
                        "platform!"
                    )

                for secondaryArchitecture in self.secondaryTargetArchitectures:
                    self.secondaryTargetMachineTriples[secondaryArchitecture] = MachineArchitecture.getTripleFor(
                        secondaryArchitecture
                    )

                if not Configuration.getSecondaryCrossDevelPackage(self.secondaryTargetArchitectures[0]):
                    sysExit(
                        "The Haiku cross devel package for all secondary "
                        "architectures must be specified on this build "
                        "platform!"
                    )
        ...
        if targetArchitecture is None:
            sysExit("TARGET_ARCHITECTURE must be set in configuration on this build platform!")
        self.targetMachineTriple = MachineArchitecture.getTripleFor(targetArchitecture)
    ...
    def activateBuildPackage(self, workDir, packagePath, revisionedName):
        return self._activatePackage(packagePath, self._getPackageInstallRoot(workDir, packagePath), None, True)
    ...
    def getCrossToolsBinPaths(self, workDir):
        return [self._getCrossToolsPath(workDir) + "/bin", self.getCrossToolsBasePrefix(workDir) + "/boot/system/bin"]
    ...
    def setupNonChrootBuildEnvironment(self, workDir, secondaryArchitecture, requiredPackages):
        ...
        for package in requiredPackages:
            self._activatePackage(package, self._getPackageInstallRoot(workDir, package), "/boot/system")

    def cleanNonChrootBuildEnvironment(self, workDir, secondaryArchitecture, buildOK):
        # remove the symlinks we created in the cross tools tree
        sysrootDir = self.getCrossSysrootDirectory(workDir)
        targetArchitecture = secondaryArchitecture if secondaryArchitecture else self.targetArchitecture
        toolsMachineTriple = self._getTargetMachineTriple(secondaryArchitecture)

        if targetArchitecture == "x86_gcc2":
            # gcc 2: uses 'sys-include' and 'lib' in the target machine dir
            toolsMachineDir = self._getOriginalCrossToolsDir(secondaryArchitecture) + "/" + toolsMachineTriple
            ...
            toolsSysrootDir = self._getOriginalCrossToolsDir(secondaryArchitecture) + "/sysroot"
    ...
    def _activateCrossTools(self, workDir, sysrootDir, secondaryArchitecture):
        ...
        toolsBinDir = self._getOriginalCrossToolsDir(secondaryArchitecture) + "/bin"
        ...
        if targetArchitecture == "x86_gcc2":
            ...
            toolsDevelopDir = self._getOriginalCrossToolsDir(secondaryArchitecture) + "/sysroot/boot/system/develop"
    ...
    def _getOriginalCrossToolsDir(self, secondaryArchitecture):
        ...
        return Configuration.getSecondaryCrossToolsDirectory(secondaryArchitecture)

    def _getCrossDevelPackage(self, secondaryArchitecture):
        if not secondaryArchitecture:
            return self.crossDevelPackage
        return Configuration.getSecondaryCrossDevelPackage(secondaryArchitecture)
    ...
    def _activatePackage(self, package, installRoot, installationLocation, isBuildPackage=False):
        ...
        if not isBuildPackage:
            ...
            args = [Configuration.getPackageCommand(), "extract", "-C", installPath, package]
            output = check_output(args).decode("utf-8")
            info(output)
        else:
            installPath = packageInfo.installPath
            if not installPath:
                sysExit('Build package "%s" doesn\'t have an install path' % package)

        # create the package links directory for the package and the .self
        # symlink
        packageLinksDir = installRoot + "/packages/" + packageInfo.versionedName
...
class BuildPlatformBuildMaster(BuildPlatform):
    ...
    def init(self, treePath, outputDirectory, packagesPath, shallowInitIsEnough=False):
        if Configuration.getTargetArchitecture() is None:
            sysExit("TARGET_ARCHITECTURE must be set in configuration for " + "build master mode!")
        if Configuration.getPackageCommand() is None:
            sysExit("--command-package must be specified for build master " + "mode!")

        super(BuildPlatformBuildMaster, self).init(
            treePath, outputDirectory, packagesPath, Architectures.ANY, "BuildMaster"
        )
