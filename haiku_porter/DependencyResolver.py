...
class DependencyResolver(object):
    ...
    def __init__(self, buildPlatform, requiresTypes, repositories, **kwargs):
        ...
        self._updateDependencies = getOption("updateDependencies") and len(repositories) > 2
        self._satisfiedPackagesCache = []

        self._populateProvidesManager()

    def determineRequiredPackagesFor(self, dependencyInfoFiles):
        packageInfos = [self._parsePackageInfo(dif, True) for dif in dependencyInfoFiles]
        ...
        while True:
            try:
                ...
                self._traversed = set([str(packageNode) for packageNode in self._pending])
                ...
                self._providesManager.removeProvidesOfPackageInfo(exception.packageNode.packageInfo)
                continue
    ...
    def _populateProvidesManager(self):
        for repository in self._repositories:
            for entry in os.listdir(repository):
                if not (entry.endswith(".DependencyInfo") or entry.endswith(".hpkg") or entry.endswith(".PackageInfo")):
                    continue
                packageInfo = self._parsePackageInfo(repository + "/" + entry, not entry.endswith(".hpkg"))
    ...
    def _buildDependencyGraph(self):
        ...
        while self._pending:
            ...
            if numberOfHandledPackages == numberOfInitialPackages and "REQUIRES" not in self._requiresTypes:
    ...
    def _sortPackageNodesTopologically(self):
        ...
        while self._packageNodes:
            ...
            self._packageNodes = [node for node in self._packageNodes if node not in nodesWithLowestDependencyCount]
    ...
    def _addAllImmediateBuildRequiresOf(self, requiredPackageInfo):
        ...
        for requires in packageInfo.buildRequires:
            self._addImmediate(requiredPackageInfo, requires, "build-requires", forBuildhost)
    ...
    def _addScriptletPrerequiresOf(self, requiredPackageInfo):
        ...
        for requires in scriptletPrerequirements:
            self._addImmediate(requiredPackageInfo, ResolvableExpression(requires), "scriptlet-prerequires", True)
    ...
    def _addImmediate(self, parent, requires, typeString, forBuildhost):
        ...
        provides = self._providesManager.getMatchingProvides(requires, isPrerequiresType, self._ignoreBase)
