...
class ProvidesManager(object):
    def __init__(self):
        ...
        self.architectures = [BuildPlatform.buildPlatform.targetArchitecture, "any", "source"]
    ...
    def addProvidesFromPackageInfo(self, packageInfo):
        if packageInfo.architecture not in self.architectures and not (
            Configuration.isCrossBuildRepository() and "_cross_" in packageInfo.path
        ):
            return
    ...
    def getMatchingProvides(self, resolvableExpression, anyHpkg=False, ignoreBase=False):
        ...
        for provides in providesList:
            provideIsHpkg = (
                provides.packageInfo.path.endswith(".hpkg") if isinstance(provides.packageInfo, PackageInfo) else False
            )
            ...
            if not operator:
                ...
                if (
                    found is None
                    or (anyHpkg and provideIsHpkg)
                    or (
                        provideIsHpkg
                        and not foundIsHpkg
                        and (
                            missingDependencies
                            or found.version is None
            ...
            if not matches:
                continue
            if provides.compatibleVersion and versionCompare(provides.compatibleVersion, version) > 0:
                continue
            if not updateDependencies and not missingDependencies:
                return provides
            if (
    ...
    def _providesSource(packageInfo):
        return packageInfo.path if isinstance(packageInfo, PackageInfo) else packageInfo
