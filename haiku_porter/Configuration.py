    ...
    def _readConfigurationFile(self):
        ...
        if not haikuportsConf:
            ...
            if not os.path.exists(haikuportsConf):
                haikuportsConf = os.path.expanduser("~") + "/config/settings/haikuports.conf"
                if not os.path.exists(haikuportsConf):
                    haikuportsConf = "/system/settings/haikuports.conf"

        if not os.path.exists(haikuportsConf):
            sysExit(
                "Unable to find haikuports.conf in known search paths.\n"
        ...
        for key in haikuportsAttributes.keys():
            ...
            if key not in configurationValue:
                if haikuportsAttributes[key]["required"]:
                    sysExit("Required value '" + key + "' not present in " + haikuportsConf)

                # set default value, as no other value has been provided
                if haikuportsAttributes[key]["default"] is not None:
                    configurationValue[key] = haikuportsAttributes[key]["default"]

            if "setAttribute" in haikuportsAttributes[key] and key in configurationValue:
        ...
        if self.isCrossBuildRepository and not self.targetArchitecture:
            sysExit("For a cross-build repository, TARGET_ARCHITECTURE needs to be set in " + haikuportsConf)
        ...
        if self.secondaryArchitectures:
            ...
            if crossTools:
                if len(crossTools) != len(self.secondaryArchitectures):
                    sysExit("A cross-tools directory must be specified for each secondary architecture")
                for architecture, tools in zip(self.secondaryArchitectures, crossTools):
            ...
            if self.secondaryCrossDevelPackages:
                ...
                if len(crossDevelPackages) != len(self.secondaryArchitectures):
                    sysExit("A cross-tools devel pacakge must be specified for each secondary architecture")
                for architecture, package in zip(self.secondaryArchitectures, crossDevelPackages):
