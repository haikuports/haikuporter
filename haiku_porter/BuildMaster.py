...
class ScheduledBuild(object):
    def __init__(self, port, portsTreePath, requiredPackageIDs, packagesPath, presentDependencyPackages):
        self.port = port
        self.recipeFilePath = os.path.relpath(port.recipeFilePath, portsTreePath)
        self.resultingPackages = [package.hpkgName for package in self.port.packages]
        self.packagesPath = packagesPath
        self.requiredPackages = presentDependencyPackages
        self.requiredPackageIDs = [os.path.basename(path) for path in presentDependencyPackages]
    ...
    def packageCompleted(self, package, available):
        ...
        if packageID in self.missingPackageIDs:
            if available:
                ...
                self.requiredPackages.append(os.path.join(self.packagesPath, package.hpkgName))
            else:
                self.lost = True
...
class SkippedBuild(object):
    def __init__(self, portsTreePath, port, reason):
        if isinstance(port, Port):
            self.port = port
            self.recipeFilePath = os.path.relpath(port.recipeFilePath, portsTreePath)
            self.resultingPackages = [package.hpkgName for package in port.packages]
        else:
            self.port = None
            self.name = port
            self.recipeFilePath = ""
    ...
    def status(self):
        return {
            "port": {
                ...
                "revisionedName": self.port.revisionedName if self.port else self.name,
                "recipeFilePath": self.recipeFilePath,
            },
            "resultingPackages": self.resultingPackages,
            "reason": self.reason,
...
class BuildMaster(object):
    def __init__(self, portsTreePath, packagesPath, options):
        ...
        logHandler = logging.FileHandler(os.path.join(self.buildOutputBaseDir, "master.log"))
        ...
        if reportURI == None:
            ...
            self.reporter = ReporterJson(reportFile, "master", Configuration.getTargetArchitecture())
            if not self.reporter.connected():
                sysExit("unable to setup json reporting engine")
        elif reportURI.startswith("mongodb://"):
            self.reporter = ReporterMongo(reportURI, "master", Configuration.getTargetArchitecture())
            if not self.reporter.connected():
        ...
        if self.localBuilders == 0:
            for fileName in os.listdir(self.builderBaseDir):
                ...
                try:
                    ...
                    self.logger.error("failed to add builder from config " + configFilePath + ":" + str(exception))
                    continue
            ...
            for i in range(0, self.localBuilders):
                builder = None
                try:
                    builder = LocalBuilder(str(i), packagesPath, self.buildOutputBaseDir, options)
                except Exception as exception:
    ...
    def schedule(self, port, requiredPackageIDs, presentDependencyPackages):
        ...
        for package in port.packages:
            ...
            self.addSkipped(port, "some packages already exist at " + self.packagesPath + ", revision bump required")
            return

        self.logger.info("scheduling build of " + port.versionedName)
        scheduledBuild = ScheduledBuild(
            port, self.portsTreePath, requiredPackageIDs, self.packagesPath, presentDependencyPackages
        )
    ...
    def _fillPortsTreeInfo(self):
        try:
            ...
            origin = subprocess.check_output(
                ["git", "remote", "get-url", "origin"], cwd=self.portsTreePath, stderr=subprocess.STDOUT
            ).decode("utf-8")
            head = subprocess.check_output(
                ["git", "rev-parse", "HEAD"], cwd=self.portsTreePath, stderr=subprocess.STDOUT
            ).decode("utf-8")
        except:
    ...
    def _runBuild(self, scheduledBuild):
        while True:
            ...
            threading.Thread(
                None, self._buildThread, "build " + str(buildNumber), (builder, scheduledBuild, buildNumber)
            ).start()
            break
    ...
    def _packagesCompleted(self, packages, available):
        ...
        with self.buildableCondition:
            ...
            while len(completePackages) > 0:
                ...
                self.logger.info(
                    "package " + package.versionedName + " " + ("became available" if available else "lost")
                )
    ...
    def _buildThread(self, builder, scheduledBuild, buildNumber):
        ...
        if not buildSuccess and reschedule:
            ...
            with open(os.path.join(self.buildRecordsDir, str(buildNumber) + ".json"), "w") as outputFile:
            ...
            self._buildComplete(
                scheduledBuild, buildSuccess, self.completeBuilds if buildSuccess else self.failedBuilds
            )
    ...
    def status(self):
        return {
            ...
            "builders": {
                "active": [builder.status for builder in self.activeBuilders if builder.currentBuild is not None],
                "reconnecting": [builder.status for builder in self.reconnectingBuilders],
                "idle": [builder.status for builder in self.activeBuilders if builder.currentBuild is None],
    ...
    def summary(self):
        ...
        if self.impulseIndex >= len(self.impulseData):
            self.impulseIndex = 0
        impulseTime = (self.impulseData[self.impulseIndex]["time"]) if self.impulseData[self.impulseIndex] else None
        impulsePkgCount = (
            (self.impulseData[self.impulseIndex]["pkgCount"]) if self.impulseData[self.impulseIndex] else None
        )
        now = time.time()
        ...
        return {
            ...
            "pkg_hour": int(pkgCount * 3600 / (now - self.startTime)) if self.startTime else None,
            "impulse": int((pkgCount - impulsePkgCount) * 3600 / (now - impulseTime)) if impulsePkgCount else None,
        }
