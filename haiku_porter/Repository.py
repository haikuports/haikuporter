...
class Repository(object):
    ...
    def __init__(
        ...
        self.inputSourcePackagesPath = self.outputDirectory + "/input-source-packages"
        ...
        self._portIdForPackageIdFilePath = self.path + "/.portIdForPackageIdMap"
        self._portNameForPackageNameFilePath = self.path + "/.portNameForPackageNameMap"

        # check repository format
        formatVersion = self._readFormatVersion()
        if formatVersion > Repository.currentFormatVersion:
            sysExit(
                "The version of the repository format used in\n\t%s"
        ...
        if (
            ...
            if formatVersion < Repository.currentFormatVersion:
                warn("Found old repository format - repopulating the repository ...")
    ...
    def activePorts(self):
        ...
        for portName in self._portVersionsByName.keys():
            ...
            if not activePortVersion:
                continue
            self._activePorts.append(self._allPorts[portName + "-" + activePortVersion])
        ...
        for version in reversed(versions):
            ...
            if not port.isBuildableOnTargetArchitecture():
                if warnAboutSkippedVersions:
                    ...
                    warn(("skipping %s, as it is %s on the target " + "architecture.") % (portID, status))
                continue
            return version
    ...
    def _initAllPorts(self):
        ...
        if os.path.exists(self.inputSourcePackagesPath):
            for fileName in sorted(os.listdir(self.inputSourcePackagesPath)):
                if not ("_source-" in fileName or "_source_rigged-" in fileName):
                    continue

                recipeFilePath = self._partiallyExtractSourcePackageIfNeeded(fileName)
                ...
                if self.outputDirectory == self.treePath:
                    ...
                    portOutputPath = self.outputDirectory + "/input-source-packages/" + name
                self._allPorts[name + "-" + version] = Port(
                    name, version, "<source-package>", portPath, portOutputPath, self.shellVariables, self.policy
                )

        # collect ports from the recipe tree
        for category in sorted(os.listdir(self.treePath)):
            categoryPath = self.treePath + "/" + category
            if not os.path.isdir(categoryPath) or category[0] == "." or "-" not in category:
                continue
            for port in sorted(os.listdir(categoryPath)):
                portPath = categoryPath + "/" + port
                portOutputPath = self.outputDirectory + "/" + category + "/" + port
                if not os.path.isdir(portPath) or port[0] == ".":
                    continue
                for recipe in os.listdir(portPath):
                    recipePath = portPath + "/" + recipe
                    if not os.path.isfile(recipePath) or not recipe.endswith(".recipe"):
                        continue
                        ...
                        if versionedName in self._allPorts:
                            ...
                            if not self.quiet and not getOption("doBootstrap"):
                                ...
                                if otherPort.category == "<source-package>":
                                    warn("%s/%s	 is overruled by input source package" % (category, versionedName))
                                else:
                                    warn(
                                        "%s/%s	 is overruled by duplicate in %s - please remove one of them"
                                        % (category, versionedName, otherPort.category)
                                    )
                            continue
                        if name not in self._portVersionsByName:
                            self._portVersionsByName[name] = [version]
                            ...
                            self._portVersionsByName[name].append(version)
                        self._allPorts[name + "-" + version] = Port(
                            name, version, category, portPath, portOutputPath, self.shellVariables, self.policy
                        )
                    else:
                        # invalid argument
                        if not self.quiet:
                            print("Warning: Couldn't parse port/version info: " + recipe)
        ...
        for portName in list(self._portVersionsByName.keys()):
            self._portVersionsByName[portName].sort(key=cmp_to_key(versionCompare))
    ...
    def _writePortForPackageMaps(self):
        ...
        try:
            with open(self._portIdForPackageIdFilePath, "w") as fh:
                json.dump(self._portIdForPackageId, fh, sort_keys=True, indent=4, separators=(",", " : "))
        except BaseException as e:
            print(e)

        try:
            with open(self._portNameForPackageNameFilePath, "w") as fh:
                json.dump(self._portNameForPackageName, fh, sort_keys=True, indent=4, separators=(",", " : "))
        except BaseException as e:
            print(e)
    ...
    def _writeFormatVersion(self):
        ...
        try:
            data = {"formatVersion": Repository.currentFormatVersion}
    ...
    def _updateRepository(self, explicitPortVersion=None, preserveFlags=True):
        ...
        for portName in sorted(self._portVersionsByName.keys(), key=str.lower):
            ...
            for version in versions:
                ...
                if os.path.exists(skippedFlag) and (
                    os.path.getmtime(port.recipeFilePath) <= os.path.getmtime(skippedFlag)
                ):
                    continue

                # update all dependency-infos of port if the recipe is newer
                # than the main dependency-info of that port
                mainDependencyInfoFile = os.path.join(self.path, port.dependencyInfoName)
                if os.path.exists(mainDependencyInfoFile) and (
                    os.path.getmtime(port.recipeFilePath) <= os.path.getmtime(mainDependencyInfoFile)
                ):
                ...
                try:
                    port.parseRecipeFile(False, forceAllowUnstable, forceAllowUnstable)

                    if not port.isBuildableOnTargetArchitecture(forceAllowUnstable):
                        touchFile(skippedFlag)
                        if not self.quiet:
                            status = port.statusOnTargetArchitecture
                            print(("\t%s is still marked as %s on target " + "architecture") % (portID, status))
                        continue
        ...
        for portID, port in updatedPorts.items():
            for package in port.packages:
                self._portIdForPackageId[package.versionedName] = port.versionedName
                self._portNameForPackageName[package.name] = port.name
            activePorts.append(portID)
    ...
    def _removeStaleDependencyInfos(self, activePorts):
        ...
        for dependencyInfo in dependencyInfos:
            ...
            packageID = dependencyInfoFileName[: dependencyInfoFileName.rindex(".")]
    ...
    def _partiallyExtractSourcePackageIfNeeded(self, sourcePackageName):
        ...
        sourcePackagePath = self.inputSourcePackagesPath + "/" + sourcePackageName
        ...
        if name.endswith("_source_rigged"):
            ...
            name = name[:-7]
        relativeBasePath = "develop/sources/%s-%s-%s" % (name, version, revision)
        recipeName = name + "-" + version + ".recipe"
        recipeFilePath = self.inputSourcePackagesPath + "/" + relativeBasePath + "/" + recipeName

        if not os.path.exists(recipeFilePath) or (
            os.path.getmtime(recipeFilePath) <= os.path.getmtime(sourcePackagePath)
        ):
            ...
            entries = (
                check_output([Configuration.getPackageCommand(), "list", "-p", sourcePackagePath])
                .decode("utf-8")
                .splitlines()
            )
            entries = [entry for entry in entries if entry in allowedEntries]
            check_call(
                [Configuration.getPackageCommand(), "extract", "-C", self.inputSourcePackagesPath, sourcePackagePath]
                + entries
            )
    ...
    def checkRepositoryConsistency(self, verbose):
        ...
        resolver = DependencyResolver(None, Port.requiresTypes, repositories, quiet=True)

        for port in sorted(self.activePorts, key=lambda port: port.name):
            for package in port.packages:
                if verbose:
                    print("checking package {} of {}".format(package.revisionedName, port.versionedName))

                try:
                    resolver.determineRequiredPackagesFor([package.dependencyInfoFile(self.path)])
                except LookupError as error:
                    print("{}:\n{}\n".format(package.revisionedName, prefixLines("\t", str(error))))
