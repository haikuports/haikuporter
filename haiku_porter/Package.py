...
class Package(object):
    def __init__(self, packageType, name, port, recipeKeys, policy, isRiggedSourcePackage=False):
        ...
        if packageType == PackageType.SOURCE:
            ...
            port.secondaryArchitecture is not None
            and port.secondaryArchitecture in self.recipeKeys["SECONDARY_ARCHITECTURES"]
            ...
            sysExit("package %s cannot be built for architecture %s" % (self.versionedName, port.targetArchitecture))
        ...
        self.targetMachineTripleAsName = port.shellVariables.get("targetMachineTripleAsName", "")
    ...
    def isBuildableOnSecondaryArchitecture(self, architecture, secondaryArchitecture, forceAllowUnstable=False):
        status = self.getStatusOnSecondaryArchitecture(architecture, secondaryArchitecture)
        allowUntested = Configuration.shallAllowUntested()
    ...
    def generateDependencyInfoWithoutProvides(self, dependencyInfoPath, requiresToUse):
        ...
        self._generateDependencyInfo(
            dependencyInfoPath, requiresToUse, fakeProvides=True, architectures=Architectures.ANY
        )
    ...
    def makeHpkg(self, requiresUpdater):
        ...
        if requiresUpdater and self.type != PackageType.SOURCE:
            ...
            self.recipeKeys["UPDATED_REQUIRES"] = requiresUpdater.updateRequiresList(requiresList)
            requiresName = "UPDATED_REQUIRES"
        else:
            requiresName = "REQUIRES"

        self._generatePackageInfo(
            self.packagingDir + "/.PackageInfo", [requiresName], getOption("quiet"), False, True, self.architecture
        )
        ...
        output = check_output([Configuration.getPackageCommand(), "create", packageFile], cwd=self.packagingDir).decode(
            "utf-8"
        )
        info(output)
        # policy check
    ...
    def createBuildPackage(self):
        ...
        buildPackageInfo = self.buildPackageDir + "/" + self.revisionedName + "-build.PackageInfo"
        self._generatePackageInfo(
            buildPackageInfo, ["REQUIRES", "BUILD_REQUIRES", "BUILD_PREREQUIRES"], True, False, False, self.architecture
        )

        # create the build package
        buildPackage = self.buildPackageDir + "/" + self.revisionedName + "-build.hpkg"
        cmdlineArgs = [
        ...
        try:
            ...
            raise Exception("failure creating the build package: " + exception.output[:-1].decode("utf-8"))
        info(output)
    ...
    def removeBuildPackage(self):
        ...
        if self.activeBuildPackage:
            buildPlatform.deactivateBuildPackage(self.workDir, self.activeBuildPackage, self.revisionedName)
            self.activeBuildPackage = None
        if self.buildPackage and os.path.exists(self.buildPackage):
            os.remove(self.buildPackage)
            self.buildPackage = None

    def _generatePackageInfo(
        self, packageInfoPath, requiresToUse, quiet, fakeEmptyProvides, withActivationActions, architecture
    ):
        ...
        with codecs.open(packageInfoPath, "w", "utf-8") as infoFile:
            ...
            infoFile.write('summary\t\t\t"' + escapeForPackageInfo(self.recipeKeys["SUMMARY"]) + '"\n')

            infoFile.write('description\t\t"')
            infoFile.write(escapeForPackageInfo("\n".join(self.recipeKeys["DESCRIPTION"])))
            infoFile.write('"\n')
            ...
            for requiresKey in requiresToUse:
                if requiresKey == "SCRIPTLET_PREREQUIRES":
                    ...
                    requiresForKey = getScriptletPrerequirements(targetMachineTripleAsName)
                else:
            ...
            self._writePackageInfoListQuotePaths(infoFile, self.recipeKeys["HOMEPAGE"], "urls")

            if withActivationActions:
                self._writePackageInfoListQuotePaths(
                    infoFile, self.recipeKeys["GLOBAL_WRITABLE_FILES"], "global-writable-files"
                )
                self._writePackageInfoListQuotePaths(
                    infoFile, self.recipeKeys["USER_SETTINGS_FILES"], "user-settings-files"
                )
                ...
                self._writePackageInfoListQuotePaths(
                    infoFile, self.recipeKeys["POST_INSTALL_SCRIPTS"], "post-install-scripts"
                )
            ...
            for index in sorted(list(self.recipeKeys["SOURCE_URI"].keys()), key=cmp_to_key(naturalCompare)):
                uricount = 1
    ...
    def _generateDependencyInfo(self, dependencyInfoPath, requiresToUse, **kwargs):
        ...
        with codecs.open(dependencyInfoPath, "w", "utf-8") as infoFile:
            ...
            json.dump(dependencyInfo, infoFile, sort_keys=True, indent=4, separators=(",", " : "))
            infoFile.write("\n")
    ...
    def populatePackagingDir(self, port):
        ...
        if port.patchesDir and os.path.exists(port.patchesDir):
            ...
            for patchFileName in os.listdir(port.patchesDir):
                if not (
                    patchFileName.startswith(port.versionedName + ".")
        ...
        if os.path.exists(Configuration.getTreePath() + "/.git"):
            try:
                ...
                haikuportsRev = check_output(
                    ["git", "rev-parse", "--short", "HEAD"], cwd=Configuration.getTreePath(), stderr=STDOUT
                ).decode("utf-8")
            except:
        ...
        with open(targetBaseDir + "/ReadMe", "w") as readmeFile:
            readmeFile.write(
                ...
                "haikuporter-URL: %s\n" % (port.name, haikuportsRepoUrl, haikuportsRev.strip(), haikuporterRepoUrl)
            )
