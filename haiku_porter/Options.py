...
def parseOptions():
    ...
    parser = OptionParser(usage="usage: %prog [options] portname[-portversion]", version="%prog " + __version__)

    basic_actions = OptionGroup(parser, "Basic Actions", "Basic haikuporter actions")

    basic_actions.add_option(
        "-l", "--list", action="store_true", dest="list", default=False, help="list available ports"
    )
    basic_actions.add_option(
        "--list-packages", action="store_true", dest="listPackages", default=False, help="list available packages"
    )
    basic_actions.add_option(
        "-a", "--about", action="store_true", dest="about", default=False, help="show description of the specified port"
    )

    basic_actions.add_option(
        "-s", "--search", action="store_true", dest="search", default=False, help="search for a port (regex)"
    )
    basic_actions.add_option(
        "--search-packages",
        action="store_true",
    ...
    basic_actions.add_option(
        ...
        default=False,
        help="analyze dependencies between ports and print information; no port parameter required",
    )
    basic_actions.add_option(
        "-c",
        "--clean",
    ...
    basic_actions.add_option(
        "-g", "--get", action="store_true", dest="get", default=False, help="get/update the ports tree"
    )
    basic_actions.add_option(
        "-t",
        "--tree",
    ...
    basic_actions.add_option(
        "--lint", action="store_true", dest="lint", default=False, help="scan the ports tree for problems"
    )
    basic_actions.add_option(
        "--list-dependencies",
        action="store_true",
    ...
    basic_flags.add_option(
        "-q", "--quiet", action="store_true", dest="quiet", default=False, help="suppress output from build actions"
    )
    basic_flags.add_option(
        "-v", "--verbose", action="store_true", dest="verbose", default=False, help="show why the recipe is broken"
    )
    basic_flags.add_option(
        "-y", "--yes", action="store_true", dest="yes", default=False, help="answer yes to all questions"
    )
    basic_flags.add_option(
        "-j",
        "--jobs",
    ...
    basic_flags.add_option(
        ...
        dest="strictPolicy",
        default=False,
        help="require strict packaging policy adherence; packaging will fail on any policy violation",
    )
    basic_flags.add_option(
        "--debug", action="store_true", dest="debug", default=False, help="show Python stack traces for fatal errors"
    )
    ...
    basic_flags.add_option(
        ...
        dest="patchFilesOnly",
        default=False,
        help="don't build the port, just download, unpack and apply patch files; don't call PATCH() though",
    )
    basic_flags.add_option(
        "-b",
        "--nobuild",
    ...
    basic_flags.add_option(
        ...
        default=False,
        help="build or update required dependencies (stop on hpkg), then build the port",
    )
    basic_flags.add_option(
    ...
    basic_flags.add_option(
        "--test", action="store_true", dest="test", default=False, help="run tests on resulting binaries"
    )
    basic_flags.add_option(
        "-C",
        "--nochroot",
        ...
        dest="chroot",
        default=True,
        help="build without a chroot()-environment - meant for debugging the build/install process",
    )
    basic_flags.add_option(
        "-f",
        "--force",
    ...
    basic_flags.add_option(
        ...
        dest="why",
        default="",
        help="determine why the given port is pulled in as a dependency of the port to be built",
    )
    ...
    basic_flags.add_option(
        ...
        dest="configFile",
        default=None,
        help='specifies the location of the global config file; the default is "~/config/settings/haikuports.conf"',
    )
    ...
    basic_flags.add_option(
        ...
        dest="printFilenames",
        default=False,
        help="print filenames instead of " + "package names in package listings and searches",
    )
    ...
    advanced_flags.add_option(
        ...
        default=None,
        help='path to the directory containing the well-known licenses; the default is "<systemDir>/data/licenses"',
    )
    advanced_flags.add_option(
        "--system-mimedb",
        action="store",
        ...
        dest="systemMimeDB",
        default=None,
        help='path to the directory containing the system MIME DB; the default is "<systemDir>/data/mime_db"',
    )
    advanced_flags.add_option(
        "--command-mimeset",
        action="store",
    ...
    advanced_flags.add_option(
        ...
        dest="crossTools",
        default=None,
        help="specifies the path to the cross-tools directory created by the Haiku build system's configure script",
    )
    ...
    advanced_flags.add_option(
        ...
        default=False,
        help="check consistency of package repository by" + " dependency solving all packages",
    )
    advanced_flags.add_option(
    ...
    buildmaster_flags = OptionGroup(
        parser, "Build Master Options", "Options only relevant to Haikuporter running in build master mode"
    )
    ...
    buildmaster_flags.add_option(
        "--console", action="store_true", dest="display", default=False, help="display a build master curses console"
    )
    ...
    # some normalization
    if getOption("patchFilesOnly") or not getOption("patch") or getOption("extractPatchset"):
