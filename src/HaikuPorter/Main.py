...
from .Utils import ensureCommandIsAvailable, haikuportsRepoUrl, info, sysExit, warn
...
class Main(object):
    ...
    def run(self, args):
        ...
        if not self.options.noSystemPackages and self.options.systemPackagesDirectory is not None:
            self.packageRepositories.append(self.options.systemPackagesDirectory)
        ...
        # init build platform
        buildPlatform.init(self.treePath, self.outputDirectory, self.packagesPath, self.shallowInitIsEnough)
        ...
        if self.options.buildMaster:
            ...
            self.buildMaster = BuildMaster(self.treePath, self.packagesPath, self.options)

            self.options.allDependencies = True
            self.options.noPackageObsoletion = True
            self.options.ignoreMessages = True

        if self.options.repositoryUpdate or self.options.checkRepositoryConsistency:
        ...
        if (
            self.options.prunePackageRepository
            or self.options.createPackageRepository
            or self.options.checkPackageRepositoryConsistency
        ):
            self.options.noPackageObsoletion = True
            ...
            packageRepository = PackageRepository(
                self.packagesPath, self.repository, self.options.quiet, self.options.verbose
            )
            ...
            if self.options.createPackageRepository:
                packageRepository.createPackageRepository(self.options.createPackageRepository)
            return
        ...
        if self.options.lint:
            if not buildPlatform.isHaiku and Configuration.getLicensesDirectory() is None:
                sysExit("LICENSES_DIRECTORY must be set in configuration on this build platform!")
            self._createRepositoryIfNeeded(True)
            if not args:
        ...
        if self.options.list or self.options.listPackages:
            ...
            if self.options.list:
                allNames = self.repository.searchPorts(None, self.options.printFilenames)
            else:
                allNames = self.repository.searchPackages(None, self.options.printFilenames)
        ...
        if self.options.search or self.options.searchPackages:
            ...
            for arg in args:
                if self.options.search:
                    ...
                    packageNames = self.repository.searchPackages(arg, self.options.printFilenames)
        ...
        if self.options.portsForFiles:
            ...
            files = [arg if os.path.isabs(arg) else os.path.join(self.treePath, arg) for arg in args]
        ...
        if self.options.portsfile:
            ...
            for port in ports:
                ...
                if portSpec["version"]:
                    portsfileAsRequires.append(portSpec["name"] + " ==" + portSpec["version"])
                else:
        ...
        if self.options.portsfile:
            ...
            self.portSpecs.append(self._splitPortSpecIntoNameVersionAndRevision(metaPortSpec))
            ...
            for portId in portsToBuild:
                ...
                if mainPackage and os.path.exists(self.packagesPath + "/" + mainPackage.hpkgName):
                    print("skipping port %s, since its main package already exists" % portId)
                    continue
            ...
            self.portSpecs = [self._splitPortSpecIntoNameVersionAndRevision(port) for port in portsNotYetBuilt]
        else:
            # if there is no argument given, exit
            if not args:
                sysExit("You need to specify a search string.\nInvoke '" + sys.argv[0] + " -h' for usage information.")
            self.portSpecs = [self._splitPortSpecIntoNameVersionAndRevision(port) for port in args]
        ...
        for portSpec in self.portSpecs:
            ...
            if portName not in portVersionsByName:
                ...
                if Configuration.isCrossBuildRepository():
                    nameWithTargetArch = portName + "_" + self.shellVariables["targetArchitecture"]
                ...
                if not portNameFound:
                    portName = self.repository.getPortNameForPackageName(portName)
                    if not portName:
                        if self.options.buildMaster:
                            self.buildMaster.addSkipped(portSpec["name"], "not found in repository")
                            continue
            ...
            if portSpec["version"]:
                ...
                if not version:
                    if self.options.buildMaster:
                        self.buildMaster.addSkipped(
                            portSpec["name"], "no version of " + portSpec["name"] + " can be built"
                        )
                        continue
                    else:
        ...
        if self.options.why:
            ...
            whySpec = self._splitPortSpecIntoNameVersionAndRevision(self.options.why)
            if not whySpec["version"]:
                whySpec["version"] = self.repository.getActiveVersionOf(whySpec["name"], False)
        ...
        for portSpec in self.portSpecs:
            ...
            if self.options.clean:
                ...
                (self.options.build and portSpec["id"] not in bootstrapPorts) or self.options.test
    ...
    def _listDependencies(self, port):
        ...
        buildDependencies = port.resolveDependencies(
            self.packageRepositories, self.options.test, presentDependencyPackages
        )

        print("packages already present:")
        presentDependencyPackageNames = [os.path.basename(package) for package in presentDependencyPackages]
    ...
    def _validateMainPort(self, port, revision=None):
        ...
        if revision and port.revision != revision:
            sysExit(
                ("Port %s isn't available in revision %s (found revision " + "%s instead)")
        ...
        if not port.isBuildableOnTargetArchitecture():
            ...
            message = "Port {} is {} on this architecture.".format(port.versionedName, status)
            warn(message)
            if self.options.buildMaster:
    ...
    def _buildMainPort(self, port, testPort):
        ...
        if self.options.buildMaster:
            presentDependencyPackages = []
            try:
                buildDependencies = port.resolveDependencies(self.packageRepositories, False, presentDependencyPackages)
            except Exception as exception:
                self.buildMaster.addSkipped(port, "resolving build dependencies failed: {}".format(exception))
                return
        else:
            buildDependencies = port.resolveDependencies(self.packageRepositories, testPort)
        ...
        for dependency in buildDependencies:
            ...
            try:
                ...
                if portID not in requiredPortIDs:
                    ...
                    if (getOption("createSourcePackagesForBootstrap") or getOption("createSourcePackages")) and (
                        not requiredPort.sourcePackage or requiredPort.sourcePackageExists(self.packagesPath)
                    ):
                        continue
        ...
        if requiredPortsToBuild:
            ...
            for requiredPort in requiredPortsToBuild:
                print("\t" + requiredPort.category + "::" + requiredPort.versionedName)
            for requiredPort in requiredPortsToBuild:
                if self.options.buildMaster:
                    ...
                    try:
                        ...
                        sysExit("Dependency of " + port.versionedName + " cannot be built")
                else:
                    self._buildPort(requiredPort, True, False)

        if self.options.buildMaster:
            self.buildMaster.schedule(port, requiredPackageIDs, presentDependencyPackages)
    ...
    def _setupForPossiblyObsoletePort(self, port):
        ...
        if port.version != activeVersion:
            ...
            warn("building obsolete port, packages will be put in {}".format(targetPath))
    ...
    def _initGlobalShellVariables(self):
        ...
        if Configuration.isCrossBuildRepository():
            ...
            if not targetHaikuPackage:
                if not buildPlatform.isHaiku:
                    sysExit(
                        "On this platform a haiku cross devel package must be specified (via --cross-devel-package)"
                    )
                targetHaikuPackage = (
                    "/boot/system/develop/cross/" + "haiku_cross_devel_sysroot_%s.hpkg"
                ) % targetArchitecture
        else:
            if (
                not buildPlatform.isHaiku
                and not self.shallowInitIsEnough
                and not (getOption("createSourcePackagesForBootstrap") or getOption("createSourcePackages"))
            ):
                sysExit("Native building not supported on this platform (%s)" % buildPlatform.name)
        ...
        if Configuration.isCrossBuildRepository():
            ...
            targetMachineTriple = MachineArchitecture.getTripleFor(targetArchitecture)
            ...
            self.shellVariables["buildMachineTripleAsName"] = buildMachineTriple.replace("-", "_")
            self.shellVariables["targetArchitecture"] = targetArchitecture
            self.shellVariables["targetMachineTriple"] = targetMachineTriple
            self.shellVariables["targetMachineTripleAsName"] = targetMachineTriple.replace("-", "_")
        else:
    ...
    def _checkFormatVersions(self):
        ...
        if os.path.exists(formatVersionsFile):
            ...
            recipeFormatVersionMatch = re.search("^RecipeFormatVersion=(.+?)$", formatVersions, flags=re.MULTILINE)
        ...
        if recipeFormatVersion > getRecipeFormatVersion():
            sysExit(
                "The version of the recipe file format used in the ports "
