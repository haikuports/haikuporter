...
class Resolvable(object):
    ...
    versionPattern = re.compile(r"([^\s=]+)\s*(=\s*([^\s]+)\s*" + r"((\(compatible|compat)\s*>=\s*([^\s)]+))?)?")
...
class PackageInfo(object):
        ...
        with open(cls.hpkgCachePath, "rb") as cacheFile:
            while True:
                try:
                    ...
                    path = entry["path"]
                    if not os.path.exists(path) or os.path.getmtime(path) > entry["modifiedTime"]:
                        prune = True
                        continue
    ...
    def _parseFromDependencyInfoFile(self):
        ...
        self.buildRequires = [ResolvableExpression(r) for r in dependencyInfo["buildRequires"]]
        self.buildPrerequires = [ResolvableExpression(r) for r in dependencyInfo["buildPrerequires"]]
        self.testRequires = [ResolvableExpression(r) for r in dependencyInfo["testRequires"]]
