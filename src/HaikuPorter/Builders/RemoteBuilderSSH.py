...
class RemoteBuilderSSH(object):
    def __init__(self, configFilePath, packagesPath, outputBaseDir, portsTreeOriginURL, portsTreeHead):
        ...
        logHandler = logging.FileHandler(os.path.join(self.builderOutputDir, self.name + ".log"), encoding="utf-8")
    ...
    def _loadConfig(self, configFilePath):
        ...
        if "privateKeyFile" not in self.config["ssh"]:
            raise Exception("missing ssh privateKeyFile config for builder " + self.name)
        ...
        if "packagesPath" not in self.config["portstree"]:
            self.config["portstree"]["packagesPath"] = self.config["portstree"]["path"] + "/packages"
        if "packagesCachePath" not in self.config["portstree"]:
            self.config["portstree"]["packagesCachePath"] = self.config["portstree"]["packagesPath"] + "/.cache"
        if "builderConfig" not in self.config["portstree"]:
            self.config["portstree"]["builderConfig"] = self.config["portstree"]["path"] + "/builder.conf"
    ...
    def _connect(self):
        try:
            ...
            if self.connectionErrors >= self.maxConnectionErrors:
                self.logger.error(
                    "giving up on builder after " + str(self.connectionErrors) + " consecutive connection errors"
                )
    ...
    def _syncPortsTree(self):
        try:
            command = 'cd "' + self.config["portstree"]["path"] + '" && git fetch && git checkout ' + self.portsTreeHead
            self.logger.info("running command: " + command)
            (output, channel) = self._remoteCommand(command)
    ...
    def _writeBuilderConfig(self):
        try:
            config = {
                ...
                "PACKAGER": "Builder " + self.name + " <hpkg-builder@haiku-os.org>",
                "TARGET_ARCHITECTURE": Configuration.getTargetArchitecture(),
                "SECONDARY_TARGET_ARCHITECTURES": Configuration.getSecondaryTargetArchitectures(),
            ...
            with self._openRemoteFile(self.config["portstree"]["builderConfig"], "w") as remoteFile:
    ...
    def _removeObsoletePackages(self):
        ...
        for entry in list(self.availablePackages):
            if not os.path.exists(os.path.join(self.packagesPath, entry)):
                self.logger.info("removing obsolete package {} from cache".format(entry))
    ...
    def setBuild(self, scheduledBuild, buildNumber):
        logHandler = logging.FileHandler(os.path.join(self.buildOutputDir, str(buildNumber) + ".log"), encoding="utf-8")
    ...
    def runBuild(self):
        ...
        try:
            ...
            for package in scheduledBuild.port.packages:
                self.buildLogger.info("download package " + package.hpkgName + " from builder")

                packageFile = os.path.join(self.packagesPath, package.hpkgName)
                downloadFile = packageFile + ".download"
                self._getFile(self.config["portstree"]["packagesPath"] + "/" + package.hpkgName, downloadFile)
    ...
    def _move(self, sourcePath, destPath):
        ...
        (output, channel) = self._remoteCommand('mv "' + sourcePath + '" "' + destPath + '"')
        if channel.recv_exit_status() != 0:
    ...
    def _makePackageAvailable(self, packagePath):
        ...
        entryPath = self.config["portstree"]["packagesCachePath"] + "/" + packageName
    ...
    def status(self):
        return {
            ...
            "currentBuild": (
                {"build": self.currentBuild["status"], "number": self.currentBuild["number"]}
