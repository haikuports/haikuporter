...
from .SourceFetcher import (
    createSourceFetcher,
    foldSubdirIntoSourceDir,
    parseCheckoutUri,
)
from .Utils import (
...
class Source(object):
    def __init__(self, port, index, uris, fetchTargetName, checksum, sourceDir, patches, additionalFiles):
        self.index = index
        self.uris = uris
        ...
        if self.additionalFiles:
            self.additionalFiles = [
                port.additionalFilesDir + "/" + additionalFile for additionalFile in self.additionalFiles
            ]
        ...
        if downloadMirror:
            ...
            self.uris.append(downloadMirror + "/" + recipeDirName + "/" + uriFileName + uriExtension)
    ...
    def fetch(self, port):
        ...
        if os.path.exists(self.fetchTarget):
            if os.path.exists(uriFile):
                ...
                (unusedType, storedBaseUri, storedRev) = parseCheckoutUri(storedUri)

                for uri in self.uris:
                    (unusedType, baseUri, rev) = parseCheckoutUri(uri)
                    if baseUri == storedBaseUri:
                        self.sourceFetcher = createSourceFetcher(uri, self.fetchTarget)
                        if rev != storedRev:
                            ...
                            info("Skipping download of source for " + self.fetchTargetName)
                        break
                else:
                    warn("Stored SOURCE_URI is no longer in recipe, automatic repository update won't work")
                    self.sourceFetcher = createSourceFetcher(storedUri, self.fetchTarget)
        ...
        for uri in self.uris:
            try:
                ...
                if isinstance(e, CalledProcessError):
                    info(e.output)
                if uri != self.uris[-1]:
                    warn(("Unable to fetch source from %s (error: %s), " + "trying next location.") % (uri, e))
                else:
    ...
    def unpack(self, port):
        ...
        self.sourceFetcher.unpack(self.sourceBaseDir, self.sourceSubDir, self.sourceExportSubdir)
    ...
    def populateAdditionalFiles(self, baseDir):
        ...
        for additionalFile in self.additionalFiles:
            if os.path.isdir(additionalFile):
                shutil.copytree(additionalFile, os.path.join(additionalFilesDir, os.path.basename(additionalFile)))
            else:
    ...
    def validateChecksum(self, port):
        ...
        if self.checksum is not None:
            ...
            warn(
                ...
                % {"digest": sha256.hexdigest(), "index": ("_" + self.index) if self.index != "1" else ""}
            )
            warn("-----------------------------")
    ...
    def isFromRiggedSourcePackage(self):
        ...
        return self.uris[0].lower().startswith("pkg:") and "_source_rigged-" in self.uris[0].lower()
            ...
            for additionalFile in self.additionalFiles:
                if os.path.isdir(additionalFile):
                    ...
                    for fileName in files:
                        if os.path.commonprefix([additionalFile, fileName]) == additionalFile:
                            return True
        ...
        try:
            ...
            for patch in self.patches:
                ...
                if getOption("noGitRepo"):
                    ...
                    output = check_output(
                        ["patch", "--ignore-whitespace", "-p1", "-i", patch], cwd=self.sourceDir
                    ).decode("utf-8")
                    info(output)
                else:
                    if patch.endswith(".patchset"):
                        ...
                        output = check_output(
                            ["git", "apply", "--ignore-whitespace", "-p1", "--index", patch], cwd=self.sourceDir
                        ).decode("utf-8")
                        info(output)
                        output = check_output(
                            ["git", "commit", "-q", "-m", "applying patch %s" % os.path.basename(patch)],
                            cwd=self.sourceDir,
                            env=self.gitEnv,
                        ).decode("utf-8")
                        info(output)
    ...
    def commitPatchPhase(self):
        ...
        changes = check_output(["git", "status", "--porcelain"], cwd=self.sourceDir).decode("utf-8")
        if not changes:
        ...
        output = check_output(
            ["git", "commit", "-a", "-q", "-m", "patch function"], cwd=self.sourceDir, env=self.gitEnv
        ).decode("utf-8")
        info(output)
        output = check_output(["git", "tag", "-f", "PATCH_FUNCTION", "HEAD"], cwd=self.sourceDir).decode("utf-8")
        info(output)
    ...
    def extractPatchset(self, patchSetFilePath, archPatchSetFilePath):
        ...
        if not os.path.exists(self.sourceDir):
            sysExit("Can't extract patchset for " + self.sourceDir + " as the source directory doesn't exist yet")
        ...
        try:
            ...
            with open(os.devnull, "w") as devnull:
                check_call(
                    ["git", "rev-parse", "--verify", "PATCH_FUNCTION"],
                    stdout=devnull,
                    stderr=devnull,
                    cwd=self.sourceDir,
                )
        ...
        if needToRebase:
            ...
            check_call(
                ["git", "rebase", "-q", "--onto", "PATCH_FUNCTION^", "PATCH_FUNCTION", "haikuport"],
                cwd=self.sourceDir,
                env=self.gitEnv,
            )
        ...
        with open(patchSetFilePath, "w") as patchSetFile:
            check_call(
                ["git", "format-patch", "-kp", "--stdout", "ORIGIN"],
                stdout=patchSetFile,
                cwd=self.sourceDir,
                env=self.gitEnv,
            )
        ...
        if needToRebase:
            ...
            check_call(["git", "rebase", "-q", "PATCH_FUNCTION", "haikuport"], cwd=self.sourceDir, env=self.gitEnv)

        # warn if there's a correpsonding arch-specific patchset file
        if os.path.exists(archPatchSetFilePath):
            warn("arch-specific patchset file %s requires manual update" % os.path.basename(archPatchSetFilePath))
    ...
    def exportSources(self, targetDir, rigged):
        ...
        if rigged:
            ...
            check_call("tar c --exclude=.git . | tar x -C %s" % targetDir, cwd=self.sourceDir, shell=True)
        else:
            # unpack the archive into the targetDir
            if self.sourceSubDir:
                os.mkdir(targetDir + "/" + self.sourceSubDir)
            self.sourceFetcher.unpack(targetDir, self.sourceSubDir, self.sourceExportSubdir)
            if self.sourceSubDir:
    ...
    def adjustToChroot(self, port):
        ...
        self.additionalFiles = [additionalFile[pathLengthToCut:] for additionalFile in self.additionalFiles]
    ...
    def _initImplicitGitRepo(self):
        ...
        info(check_output(["git", "symbolic-ref", "HEAD", "refs/heads/haikuport"], cwd=self.sourceDir).decode("utf-8"))
        info(check_output(["git", "add", "-f", "."], cwd=self.sourceDir).decode("utf-8"))
        info(check_output(["git", "commit", "-m", "import", "-q"], cwd=self.sourceDir, env=self.gitEnv).decode("utf-8"))
