...
class ConfigParser(object):
    def __init__(self, filename, attributes, shellVariables):
        ...
        wrapperScript = getShellVariableSetters(shellVariables) + configFileEvaluatorScript
        try:
        ...
        for line in lines:
            ...
            if not separator:
                sysExit(
                    "evaluating file %s produced illegal "
                    'key-values line:\n	 %s\nexpected "<key>=<value>"\n'
            ...
            if key in attributes:
                ...
                while subKeys:
                    ...
                    if not isPhaseKey:
                        ...
                        warn("Key %s in file %s is unsupported, ignoring it" % (key, filename))
                    continue
            ...
            if attrType == bytes:
                ...
                try:
                    ...
                    sysExit(
                        'evaluating file %s produced illegal value "%s" for key %s, expected an <integer> value'
                ...
                if attrType in [ProvidesList, RequiresList]:
                    ...
                    for value in values:
                        if "-" in value.split()[0]:
                            sysExit(
                                "evaluating file %s produced illegal value "
                                '"%s" for key %s\n'
                ...
                if valueString.upper() not in Phase.getAllowedValues():
                    sysExit(
                        'evaluating file %s\nproduced illegal value "%s" for key %s\nexpected one of: %s'
                        % (filename, valueString, key, ",".join(Phase.getAllowedValues()))
                    )
                ...
                if valueString not in knownArchitectures:
                    ...
                    sysExit(
                        "%s refers to unknown machine-architecture %s\nknown machine-architectures: %s"
