...
class PortNode(object):
    ...
    def isBuildable(self, repositoryPath, doneRepositoryPath):
        ...
        if not getOption("noSystemPackages"):
            repositories.append(buildPlatform.findDirectory("B_SYSTEM_PACKAGES_DIRECTORY"))

        try:
            buildPlatform.resolveDependencies(dependencyInfoFiles, requiresTypes, repositories)
...
class DependencyAnalyzer(object):
    ...
    def printDependencies(self):
        ...
        for packageNode in sorted(self.systemPackageNodes, key=lambda packageNode: packageNode.name):
            print("	 %s" % packageNode.name)

        print("Ports required by haikuporter:")
        for packageNode in sorted(self.haikuporterRequires, key=lambda packageNode: packageNode.name):
            print("	 %s" % packageNode.portNode.name)

        print("Ports depending cyclically on each other:")
        for node in sorted(sorted(self.cyclicNodes, key=lambda node: node.name), key=lambda node: node.outdegree):
    ...
    def getBuildOrderForBootstrap(self):
        ...
        while nodes:
            ...
            if lastDoneCount == len(done):
                sysExit(
                    "None of these cyclic dependencies can be built:\n\t"
    ...
    def _doInitialDependencyResolution(self):
        ...
        for portID in allActivePorts:
            ...
            for package in portNode.port.packages:
                ...
                packageNode.addRequires(self._resolveRequiresList(packageInfo.requires, portID, packageID))
                portNode.addBuildRequires(self._resolveRequiresList(packageInfo.buildRequires, portID, packageID))
                portNode.addBuildPrerequires(self._resolveRequiresList(packageInfo.buildPrerequires, portID, packageID))
        ...
        for packageNode in self.systemPackageNodes:
            ...
            packageNode.addRequires(
                self._resolveRequiresList(packageInfo.requires, "system packages", packageNode.name)
            )
        ...
        while nodeStack:
            ...
            for dependency in packageNode.requires:
                if dependency in nonSystemPackageNodes and dependency not in self.systemPackageNodes:
                    nodeStack.append(dependency)
                    self.systemPackageNodes.add(dependency)

        # resolve the haikuporter dependencies
        scriptletPrerequirements = [ResolvableExpression(requires) for requires in getScriptletPrerequirements()]
        haikuporterDependencies = self._resolveRequiresList(
            scriptletPrerequirements, "haikuporter", "scriptlet requires"
        )
        ...
        while nodeStack:
            packageNode = nodeStack.pop()
            for dependency in packageNode.requires:
                if dependency in nonSystemPackageNodes and dependency not in self.haikuporterRequires:
                    nodeStack.append(dependency)
    ...
    def _collectDependencyInfos(self, path):
        for entry in os.listdir(path):
            ...
            try:
                ...
                print('Warning: Ignoring broken dependency-info file "%s"' % dependencyInfoFile)
    ...
    def _resolveRequiresList(self, requiresList, portID, packageID):
        ...
        for requires in requiresList:
            ...
            if providesInfo:
                isSystemPackage = buildPlatform.isSystemPackage(providesInfo.path)
                packageNode = self._getPackageNode(providesInfo.packageID, isSystemPackage)
                dependencies.add(packageNode)
            else:
                print(
                    'Warning: Ignoring unresolvable requires "%s" of package %s in %s' % (requires, packageID, portID)
                )
        return dependencies
