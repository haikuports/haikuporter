...
class ChrootSetup(object):
    ...
    def __enter__(self):
        ...
        check_output(["bash", "-c", setupChrootScript], env=shellEnv, cwd=self.path).decode("utf-8")
        return self
...
class Port(object):
    requiresTypes = ["REQUIRES", "BUILD_REQUIRES", "BUILD_PREREQUIRES", "SCRIPTLET_PREREQUIRES"]
    ...
    def __init__(
        self, name, version, category, baseDir, outputDir, globalShellVariables, policy, secondaryArchitecture=None
    ):
        self.baseName = name
        ...
        if secondaryArchitecture:
            self.workDir = outputDir + "/work-" + secondaryArchitecture + "-" + self.version
        ...
        self.recipeFilePath = self.baseDir + "/" + self.baseName + "-" + self.version + ".recipe"
    ...
    def validateRecipeFile(self, showWarnings=False):
        ...
        recipeConfig = ConfigParser(self.preparedRecipeFile, recipeAttributes, self.shellVariables)
                ...
                if extension:
                    ...
                    if key not in entries:
                        ...
                        if attributes["extendable"] == Extendable.DEFAULT:
                            ...
                            if "suffix" in attributes and extension in attributes["suffix"]:
                                recipeKeys[baseKey] = baseEntries[baseKey] + attributes["suffix"][extension]
                            else:
        ...
        if not self.isMetaPort:
            ...
            for fileExtension in ["diff", "patch", "patchset"]:
                ...
                if self.effectiveTargetArchitecture == MachineArchitecture.X86_GCC2:
                    suffixes.append("-gcc2")
                else:
                    suffixes.append("-gcc4")
                for suffix in suffixes:
                    patchFileName = "%s%s.%s" % (versionedBaseName, suffix, fileExtension)
                    if os.path.exists(self.patchesDir + "/" + patchFileName) and patchFileName not in allPatches:
                        sysExit("Patch file %s is not referenced in PATCHES, so it will not be used" % patchFileName)
    ...
    def _validateSUMMARY(self, key, entries, showWarnings):
        ...
        if key not in entries or not entries[key]:
            sysExit("No %s found (in %s)" % (key, self.recipeFilePath))
        if "\n" in entries[key]:
            sysExit("%s must be a single line of text (%s)." % (key, self.recipeFilePath))
        if entries[key].lower().startswith(self.name):
            sysExit("%s cannot start with the name of the port (%s)." % (key, self.recipeFilePath))
        if entries[key][0].islower():
            sysExit("%s must start with a capital letter (%s)." % (key, self.recipeFilePath))
    ...
    def _validateDESCRIPTION(self, key, entries, showWarnings):
        ...
        if "SUMMARY" in entries and "".join(entries[key]) == entries["SUMMARY"]:
            sysExit("%s cannot be the same as SUMMARY (in %s)" % (key, self.recipeFilePath))
    ...
    def _validateLICENSE(self, key, entries, showWarnings):
        ...
        if key in entries and entries[key]:
            ...
            for item in recipeLicense:
                haikuLicenseList = fileList = os.listdir(buildPlatform.getLicensesDirectory())
                if item not in fileList and self.licensesDir and os.path.exists(self.licensesDir):
                    fileList = []
    ...
    def _validateCOPYRIGHT(self, key, entries, showWarnings):
        ...
        if key not in entries or not entries[key]:
            ...
            sysExit("%s must not contain e-mail addresses (in %s)" % (key, self.recipeFilePath))
        else:
            lowerc = fullCopyright.lower()
            if "copyright" in lowerc or "(c)" in lowerc or "©" in lowerc:
                sysExit('%s must not contain "COPYRIGHT", "(C)", or © (in %s)' % (key, self.recipeFilePath))
    ...
    def printDescription(self):
        ...
        for package in self.allPackages:
            ...
            print("STATUS: %s" % package.getStatusOnArchitecture(self.targetArchitecture))
    ...
    def isBuildableOnTargetArchitecture(self, forceAllowUnstable=False):
        ...
        return status == Status.STABLE or (status == Status.UNTESTED and allowUntested) or forceAllowUnstable
    ...
    def resolveDependencies(self, repositories, forTestPhase, presentDependencyPackages=None):
        ...
        requiredPackages = self._resolveDependencies(
            ...
            "required or prerequired ports for {}".format("test" if forTestPhase else "build"),
    ...
    def whyIsPortRequired(self, packagesPath, requiredPort):
        ...
        symlinkGlob(os.path.join(self._repositoryDir, "*.DependencyInfo"), workRepositoryPath)
        ...
        warn("port %s doesn't seem to be required by %s" % (requiredPort.versionedName, self.versionedName))

    def getDependencyInfoFiles(self):
        """Returns the list of dependency info files for this port."""

        return [package.dependencyInfoFile(self._repositoryDir) for package in self.packages]
    ...
    def extractPatchset(self):
        ...
        for s, source in enumerate(self.sources, 1):
            if s == 1:
                ...
                archPatchSetFileName = self.name + "-" + self.version + "-" + self.targetArchitecture + ".patchset"
            else:
                patchSetFileName = self.name + "-" + self.version + "-source" + str(s) + ".patchset"
                archPatchSetFileName = (
                    self.name + "-" + self.version + "-" + self.targetArchitecture + "-source" + str(s) + ".patchset"
                )
    ...
    def build(self, packagesPath, makePackages, hpkgStoragePath):
        ...
        if (
            not getOption("preserveFlags")
            and self.checkFlag("build")
            and (os.path.getmtime(self.recipeFilePath) > os.path.getmtime(self.workDir + "/flag.build"))
        ):
        ...
        if not (getOption("createSourcePackagesForBootstrap") or getOption("createSourcePackages")):
            for package in self.packages:
                if package.type == PackageType.SOURCE:
                    continue
        ...
        if getOption("createSourcePackagesForBootstrap") or getOption("createSourcePackages"):
            requiredPackages = []
            ...
            prerequiredPackages = self._getPackagesPrerequiredForBuild(packagesPath)
            self.requiresUpdater = RequiresUpdater(self.packages, requiredPackages)
            if not Configuration.isCrossBuildRepository() and not getOption("noSystemPackages"):
                self.requiresUpdater.addPackages(buildPlatform.findDirectory("B_SYSTEM_PACKAGES_DIRECTORY"))
        ...
        if buildPlatform.usesChroot():
            ...
            if Configuration.isCrossBuildRepository():
                chrootEnvVars["crossSysrootDir"] = self.shellVariables["crossSysrootDir"]
            ...
            def makeChrootFunctions():
                ...
                return {"task": taskFunction, "success": successFunction, "failure": failureFunction}
            ...
            buildPlatform.setupNonChrootBuildEnvironment(self.workDir, self.secondaryArchitecture, allPackages)
            try:
                self._executeBuild(makePackages)
            except:
                buildPlatform.cleanNonChrootBuildEnvironment(self.workDir, self.secondaryArchitecture, False)
                raise
            buildPlatform.cleanNonChrootBuildEnvironment(self.workDir, self.secondaryArchitecture, True)
        ...
        if makePackages and not getOption("enterChroot"):
            ...
            for package in self.packages:
                if (
                    getOption("createSourcePackagesForBootstrap") or getOption("createSourcePackages")
                ) and package.type != PackageType.SOURCE:
                    continue
                packageFile = self.hpkgDir + "/" + package.hpkgName
                if os.path.exists(packageFile):
                    if not (
                        buildPlatform.usesChroot()
                        or Configuration.isCrossBuildRepository()
                        or getOption("createSourcePackagesForBootstrap")
                        or getOption("createSourcePackages")
                    ):
                        warn("not grabbing " + package.hpkgName + ", as it has not been built in a chroot.")
                        continue
                    targetPackageFile = hpkgStoragePath + "/" + package.hpkgName
                    info("grabbing " + package.hpkgName + " and moving it to " + targetPackageFile)
    ...
    def _validateOrLoadFromCache(self, showWarnings):
        ...
        recipeCacheFile = os.path.join(
            recipeCacheDir, self.name + "-" + self.version + "-" + self.effectiveTargetArchitecture
        )

        if (
            os.path.exists(self.preparedRecipeFile)
            and os.path.exists(recipeCacheFile)
            and os.path.getmtime(recipeCacheFile) >= os.path.getmtime(self.recipeFilePath)
        ):
        ...
        try:
            recipeKeysByExtension, definedPhases = self.validateRecipeFile(showWarnings)
    ...
    def _parseRecipeFile(self, showWarnings, forceAllowUnstable=False):
        ...
        self.recipeKeysByExtension, self.definedPhases = self._validateOrLoadFromCache(showWarnings)
        self.recipeKeys = {}
        ...
        for index in sorted(list(keys["SOURCE_URI"].keys()), key=cmp_to_key(naturalCompare)):
            source = Source(
                self,
                index,
                keys["SOURCE_URI"][index],
        ...
        for extension in sorted(self.recipeKeysByExtension.keys()):
            ...
            if packageType == PackageType.SOURCE:
                if (
                    not Configuration.shallCreateSourcePackages()
                    or getOption("noSourcePackages")
            ...
            if package.isBuildableOnSecondaryArchitecture(
                self.targetArchitecture, self.secondaryArchitecture, forceAllowUnstable
            ):
        ...
        if not self.isMetaPort:
            ...
            if (
                not haveSourcePackage
                and not keys["DISABLE_SOURCE_PACKAGE"]
    ...
    def _updateShellVariables(self, forParsing):
        ...
        effectiveTargetMachineTriple = MachineArchitecture.getTripleFor(self.effectiveTargetArchitecture)

        self.shellVariables["secondaryArchSubDir"] = secondaryArchSubDir
        self.shellVariables["secondaryArchSuffix"] = secondaryArchSuffix

        self.shellVariables["effectiveTargetArchitecture"] = self.effectiveTargetArchitecture
        self.shellVariables["effectiveTargetMachineTriple"] = effectiveTargetMachineTriple
        self.shellVariables["effectiveTargetMachineTripleAsName"] = effectiveTargetMachineTriple.replace("-", "_")
        ...
        if Configuration.isCrossBuildRepository():
            ...
            if "_cross_" in self.name:
                basePrefix = buildPlatform.getCrossToolsBasePrefix(self.workDir)
            else:
                installDestDir = buildPlatform.getInstallDestDir(self.workDir)
                if installDestDir:
                    self.shellVariables["installDestDir"] = installDestDir

            self.shellVariables["crossSysrootDir"] = buildPlatform.getCrossSysrootDirectory(self.workDir)
        ...
        portPackageLinksDir = (
            basePrefix + buildPlatform.findDirectory("B_PACKAGE_LINKS_DIRECTORY") + "/" + revisionedName
        )
        ...
        self.shellVariables["configureDirArgs"] = " ".join("--%s=%s" % (k.lower(), v) for k, v in configureDirs.items())

        # add one more variable containing all the dir args for CMake:
        cmakeDirArgs = {"PREFIX": prefix, "SYSCONFDIR": portPackageLinksDir + "/.settings"}
        for k, v in relativeConfigureDirs.items():
            cmakeDirArgs[k.upper()] = v
        self.shellVariables["cmakeDirArgs"] = " ".join(
            "-DCMAKE_INSTALL_%s=%s" % (k, v) for k, v in cmakeDirArgs.items()
        )

        # add another one with the list of possible variables
        self.shellVariables["configureDirVariables"] = " ".join(configureDirs.keys())
    ...
    def _recreatePackageDirectories(self):
        ...
        directoriesToCreate = [self.packageInfoDir, self.packagingBaseDir, self.buildPackageDir, self.hpkgDir]
        directoriesToRemove = [directory for directory in directoriesToCreate if os.path.exists(directory)]
    ...
    def _getNeededPackages(self, packagesPath, requiresTypes, description, forTestPhase=False):
        return self._resolveDependencies(
            self.getDependencyInfoFiles(), requiresTypes, [packagesPath], description, ignoreBase=forTestPhase
        )
    ...
    def _getPackagesNeededForScriptlets(self, packagesPath):
        ...
        return self._getNeededPackages(packagesPath, ["SCRIPTLET_PREREQUIRES"], "needed packages for scriptlets")
    ...
    def _getPackagesPrerequiredForBuild(self, packagesPath):
        ...
        return self._getNeededPackages(
            packagesPath, ["BUILD_PREREQUIRES", "SCRIPTLET_PREREQUIRES"], "prerequired packages for build"
        )
    ...
    def _getPackagesRequiredForBuild(self, packagesPath):
        ...
        return self._getNeededPackages(packagesPath, ["BUILD_REQUIRES"], "required packages for build")
    ...
    def _getPackagesPrerequiredForTest(self, packagesPath):
        ...
        return self._getNeededPackages(
            packagesPath, ["BUILD_PREREQUIRES", "SCRIPTLET_PREREQUIRES"], "prerequired packages for test", True
        )
    ...
    def _getPackagesRequiredForTest(self, packagesPath):
        ...
        return self._getNeededPackages(
            packagesPath, ["BUILD_REQUIRES", "TEST_REQUIRES"], "required packages for test", True
        )
    ...
    def _makePackages(self):
        ...
        if not (getOption("createSourcePackagesForBootstrap") or getOption("createSourcePackages")):
            ...
            for dirName in ["add-ons", "apps", "bin", "data", "develop", "documentation", "lib", "preferences"]:
    ...
    def _doInstallStage(self):
        ...
        # activate build package if required at this stage
        if self.recipeKeys["BUILD_PACKAGE_ACTIVATION_PHASE"] == Phase.INSTALL and not (
            getOption("createSourcePackagesForBootstrap") or getOption("createSourcePackages")
        ):
            for package in self.packages:
                if package.type != PackageType.SOURCE:
    ...
    def _doTestStage(self):
        ...
        # activate build package if required at this stage
        if self.recipeKeys["BUILD_PACKAGE_ACTIVATION_PHASE"] == Phase.TEST and not (
            getOption("createSourcePackagesForBootstrap") or getOption("createSourcePackages")
        ):
            for package in self.packages:
                if package.type != PackageType.SOURCE:
    ...
    def _doRecipeAction(self, action, targetDir):
        ...
        wrapperScriptContent = getShellVariableSetters(shellVariables) + recipeActionScript
    ...
    def _resolveDependencies(self, dependencyInfoFiles, requiresTypes, repositories, description, **kwargs):
        """Resolve dependencies of one or more dependency-infos"""

        try:
            return buildPlatform.resolveDependencies(dependencyInfoFiles, requiresTypes, repositories, **kwargs)
        except (CalledProcessError, LookupError):
            if getOption("buildMaster"):
                raise

            sysExit(
                ("unable to resolve %s for %s\n" + "\tdependency-infos:\n\t\t%s\n" + "\trepositories:\n\t\t%s\n")
    ...
    def _createSourcePackage(self, name, rigged):
        ...
        sourceSuffix = recipeAttributes["SUMMARY"]["suffix"][PackageType.SOURCE]
        sourceKeys.update(
            {
