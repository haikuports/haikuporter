...
def unpackCheckoutWithTar(checkoutDir, sourceBaseDir, sourceSubDir, foldSubDir):
    ...
    sourceDir = sourceBaseDir + "/" + sourceSubDir if sourceSubDir else sourceBaseDir
    if foldSubDir:
        command = 'tar -c -C "%s" --exclude-vcs | tar -x -C "%s"' % (foldSubDir, sourceDir)
    else:
...
class SourceFetcherForBazaar(object):
    ...
    def fetch(self):
        if not Configuration.shallAllowUnsafeSources():
            sysExit("Downloading from unsafe sources is disabled in " + "haikuports.conf!")
    ...
    def updateToRev(self, rev):
        warn("Updating of a Bazaar repository to a specific revision has not been implemented yet, sorry")
...
class SourceFetcherForCvs(object):
    ...
    def updateToRev(self, rev):
        warn("Updating of a CVS repository to a specific revision has not been implemented yet, sorry")
...
class SourceFetcherForFossil(object):
    ...
    def fetch(self):
        ...
        command = "fossil clone " + self.uri + " " + fossilDir + " && fossil open " + fossilDir
        if self.rev:
            command += " " + self.rev
        output = check_output(command, shell=True, stderr=STDOUT).decode("utf-8")
        info(output)
    ...
    def updateToRev(self, rev):
        warn("Updating of a Fossil repository to a specific revision has not been implemented yet, sorry")
...
class SourceFetcherForGit(object):
    ...
    def updateToRev(self, rev):
        ...
        try:
            ...
            branches = (
                check_output(command, shell=True, cwd=self.fetchTarget, stderr=STDOUT).decode("utf-8").splitlines()
            )
...
class SourceFetcherForMercurial(object):
    ...
    def unpack(self, sourceBaseDir, sourceSubDir, foldSubDir):
        ...
        sourceDir = sourceBaseDir + "/" + sourceSubDir if sourceSubDir else sourceBaseDir
        if foldSubDir:
            command = 'hg archive -r %s -I "%s" -t files "%s"' % (self.rev, foldSubDir, sourceDir)
        else:
...
class SourceFetcherForSourcePackage(object):
    ...
    def unpack(self, sourceBaseDir, sourceSubDir, foldSubDir):
        ...
        if name.endswith("_source_rigged"):
            ...
            name = name[:-7]
        relativeSourcePath = "develop/sources/%s-%s-%s/%s" % (name, version, revision, os.path.basename(sourceBaseDir))

        if not os.path.exists(sourceDir):
            os.mkdir(sourceDir)
        output = check_output(
            [Configuration.getPackageCommand(), "extract", "-C", sourceDir, self.sourcePackagePath, relativeSourcePath],
            stderr=STDOUT,
        ).decode("utf-8")
        info(output)
...
class SourceFetcherForSubversion(object):
    ...
    def updateToRev(self, rev):
        warn("Updating of a Subversion repository to a specific revision has not been implemented yet, sorry")
