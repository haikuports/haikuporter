...
class Policy(object):
    ...
    def setPort(self, port, requiredPackages):
        ...
        self.secondaryArchSuffix = "_" + self.secondaryArchitecture if self.secondaryArchitecture else ""
        self.secondaryArchSubDir = "/" + self.secondaryArchitecture if self.secondaryArchitecture else ""
        ...
        for package in port.packages:
            self.portPackagesProvides[package.name] = self._parseResolvableExpressionList(
    ...
    def _checkProvides(self):
        ...
        if os.path.exists(binDir):
            for entry in os.listdir(binDir):
                ...
                if name.lower() not in self.provides:
                    self._violation('no matching provides "%s" for "%s"' % (name, "bin/" + entry))

        # library entries in lib[/<arch>] must be declared as lib:*[_<arch>]
        libDir = os.path.join(self.package.packagingDir, "lib" + self.secondaryArchSubDir)
        if os.path.exists(libDir):
            for entry in os.listdir(libDir):
                ...
                name = self._normalizeResolvableName("lib:" + entry[:suffixIndex] + self.secondaryArchSuffix)
                if name.lower() not in self.provides:
                    self._violation('no matching provides "%s" for "%s"' % (name, "lib/" + entry))

        # library entries in develop/lib[<arch>] must be declared as
        # devel:*[_<arch>]
        developLibDir = os.path.join(self.package.packagingDir, "develop/lib" + self.secondaryArchSubDir)
        if os.path.exists(developLibDir):
            for entry in os.listdir(developLibDir):
                ...
                name = self._normalizeResolvableName("devel:" + entry[:suffixIndex] + self.secondaryArchSuffix)
                if name.lower() not in self.provides:
                    self._violation('no matching provides "%s" for "%s"' % (name, "develop/lib/" + entry))
    ...
    def _checkLibraryDependenciesOfFile(self, dirPath, path):
        ...
        try:
            with open(os.devnull, "w") as devnull:
                output = check_output(["readelf", "--dynamic", path], stderr=devnull).decode("utf-8")
        except:
            return
        ...
        for library in libraries:
            if self._isMissingLibraryDependency(library, dirPath, rpath):
                if library.startswith("libgcc") or library.startswith("libstdc++") or library.startswith("libsupc++"):
                    continue
                self._violation(
                    '"%s" needs library "%s", but the package doesn\'t seem to declare that as a requirement'
                    % (path, library)
                )
    ...
    def _isMissingLibraryDependency(self, library, dirPath, rpath):
        ...
        libDir = os.path.join(self.package.packagingDir, "lib" + self.secondaryArchSubDir + "/" + library)
        if os.path.exists(libDir):
            return False
            ...
            for rpath1 in rpath.split(":"):
                if rpath1.find("/.self/") != -1:
                    rpathDir = os.path.join(
                        self.package.packagingDir, rpath1[rpath1.find("/.self/") + len("/.self/") :] + "/" + library
                    )
                    if os.path.exists(rpathDir):
                        return False
                elif rpath1.find("$ORIGIN") != -1:
                    rpathDir = os.path.join(
                        dirPath, rpath1[rpath1.find("$ORIGIN/") + len("$ORIGIN/") :] + "/" + library
                    )
                    if os.path.exists(rpathDir):
                        return False
        ...
        resolvableName = None
        if suffixIndex >= 0:
            resolvableName = self._normalizeResolvableName("lib:" + library[:suffixIndex] + self.secondaryArchSuffix)
        ...
        if not providingPackage:
            ...
            if not providingPackage:
                print('Warning: failed to determine the package providing "%s"' % libraryPath)
                return False
    ...
    def _getPackageProvidingPath(self, path):
        try:
            with open(os.devnull, "w") as devnull:
                output = check_output(["catattr", "-d", "SYS:PACKAGE_FILE", path], stderr=devnull).decode("utf-8")
                if output.endswith("\n"):
                    output = output[:-1]
                return output
        except:
    ...
    def _getPackageProvides(self, package):
        ...
        try:
            with open(os.devnull, "w") as devnull:
                output = check_output([Configuration.getPackageCommand(), "list", package], stderr=devnull).decode(
                    "utf-8"
                )
        except:
            return None
    ...
    def _checkMisplacedDevelopLibraries(self):
        ...
        for entry in os.listdir(libDir):
            ...
            path = libDir + "/" + entry
            self._violation(
                'development library entry "%s" should be placed in "develop/lib%s"' % (path, self.secondaryArchSubDir)
            )
    ...
    def _checkGlobalWritableFiles(self):
        ...
        for item in self.package.recipeKeys["GLOBAL_WRITABLE_FILES"]:
            ...
            if components:
                ...
                if directory not in allowedWritableTopLevelDirectories:
                    self._violation('Package declares invalid global writable %s "%s"' % (fileType, components[0]))
                ...
                if updateType:
                    ...
                    if not os.path.exists(absPath):
                        self._violation(
                            'Package declares non-existent global writable %s "%s" as included' % (fileType, path)
                        )
                    elif os.path.isdir(absPath) != isDirectory:
                        self._violation(
                            'Package declares non-existent global writable %s "%s", but it\'s a %s'
        ...
        for directory in allowedWritableTopLevelDirectories:
            ...
            if os.path.exists(dir):
                self._checkGlobalWritableFilesRecursively(globalWritableFiles, fileTypes, directory)

    def _checkGlobalWritableFilesRecursively(self, globalWritableFiles, fileTypes, path):
        if path in globalWritableFiles:
            if not globalWritableFiles[path]:
                self._violation('Included "%s" declared as not included global writable %s' % (path, fileTypes[path]))
            return

        absPath = os.path.join(self.package.packagingDir, path)
        if not os.path.isdir(absPath):
            self._violation('Included file "%s" not declared as global writable file' % path)
            return

        # entry is a directory -- recurse
        for entry in os.listdir(absPath):
            self._checkGlobalWritableFilesRecursively(globalWritableFiles, fileTypes, path + "/" + entry)
    ...
    def _checkUserSettingsFiles(self):
        for item in self.package.recipeKeys["USER_SETTINGS_FILES"]:
            ...
            if not components[0].startswith("settings/"):
                self._violation('Package declares invalid user settings file "%s"' % components[0])
            ...
            if len(components) > 2:
                ...
                if not os.path.exists(template):
                    self._violation(
                        'Package declares non-existent template "%s" for user settings file "%s" as included'
    ...
    def _checkPostInstallScripts(self):
        ...
        for script in self.package.recipeKeys["POST_INSTALL_SCRIPTS"]:
            ...
            if not os.path.exists(absScript):
                self._violation('Package declares non-existent post-install script "%s"' % script)
        ...
        if os.path.exists(dir):
            for script in os.listdir(dir):
                ...
                if path not in declaredFiles:
                    self._violation('script "%s" not declared as post-install script' % path)
