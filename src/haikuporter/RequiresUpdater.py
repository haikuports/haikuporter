    ...
    def _updateRequires(self, requires):
        ...
        if not matchingProvides:
            if self.getMatchingProvides(ResolvableExpression(name)):
                sysExit('found provides for "%s", but none matching the version requirement' % requires)
            return requires
        ...
        result = "%s %s %s" % (matchingProvides.name, resultOperator, matchingProvides.version)
        if isBase:
            result += " base"
        return result
