...
class BuildPlatform(object):
    ...
    def init(
        self, treePath, outputDirectory, packagesPath, architecture, machineTriple
    ):
    ...
    def resolveDependencies(
        self, dependencyInfoFiles, requiresTypes, repositories, **kwargs
    ):
...
class BuildPlatformHaiku(BuildPlatform):
    ...
    def init(self, treePath, outputDirectory, packagesPath, shallowInitIsEnough=False):
        if not os.path.exists("/packages"):
            sysExit(
                "haikuporter needs a version of Haiku with package management support"
            )
        ...
        for entry in os.listdir(packagesDir):
            if entry == "haiku.hpkg" or (
                entry.startswith("haiku-") and entry.endswith(".hpkg")
            ):
        ...
        if not machine:
            sysExit(
                "Unsupported Haiku build platform architecture %s"
    ...
    def resolveDependencies(
        ...
        if not getOption("noSystemPackages"):
            systemPackagesDir = buildPlatform.findDirectory(
    ...
    def setupNonChrootBuildEnvironment(
        self, workDir, secondaryArchitecture, requiredPackages
    ):
        ...
        while True:
            try:
                ...
                if not activated:
                    return
            info(
                "waiting for build package %s to be %s"
...
class BuildPlatformUnix(BuildPlatform):
    ...
    def init(self, treePath, outputDirectory, packagesPath, shallowInitIsEnough=False):
        ...
        super(BuildPlatformUnix, self).init(
            treePath, outputDirectory, packagesPath, architecture, machine
        )
        ...
        if targetArchitecture is None:
            sysExit(
                "TARGET_ARCHITECTURE must be set in configuration on this "
                "build platform!"
            )
    ...
    def activateBuildPackage(self, workDir, packagePath, revisionedName):
        return self._activatePackage(
            packagePath, self._getPackageInstallRoot(workDir, packagePath), None, True
        )
    ...
    def setupNonChrootBuildEnvironment(
        ...
        for package in requiredPackages:
            self._activatePackage(
                package, self._getPackageInstallRoot(workDir, package), "/boot/system"
            )
    ...
    def cleanNonChrootBuildEnvironment(self, workDir, secondaryArchitecture, buildOK):
        ...
        sysrootDir = self.getCrossSysrootDirectory(workDir)
        targetArchitecture = (
            secondaryArchitecture if secondaryArchitecture else self.targetArchitecture
        )
        ...
        if targetArchitecture == "x86_gcc2":
            ...
            toolsSysrootDir = (
                self._getOriginalCrossToolsDir(secondaryArchitecture) + "/sysroot"
            )
    ...
    def _activatePackage(
        self, package, installRoot, installationLocation, isBuildPackage=False
    ):
...
class BuildPlatformBuildMaster(BuildPlatform):
    ...
    def init(self, treePath, outputDirectory, packagesPath, shallowInitIsEnough=False):
        if Configuration.getTargetArchitecture() is None:
            sysExit(
                "TARGET_ARCHITECTURE must be set in configuration for "
                + "build master mode!"
            )
        if Configuration.getPackageCommand() is None:
