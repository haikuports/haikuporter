...
class ProvidesManager(object):
    def __init__(self):
        ...
        self.architectures = [
            BuildPlatform.buildPlatform.targetArchitecture,
            "any",
            "source",
        ]
    ...
    def getMatchingProvides(
        self, resolvableExpression, anyHpkg=False, ignoreBase=False
    ):
        ...
        for provides in providesList:
            provideIsHpkg = (
                provides.packageInfo.path.endswith(".hpkg")
                if isinstance(provides.packageInfo, PackageInfo)
                else False
            )
