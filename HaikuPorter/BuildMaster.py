...
class BuildMaster(object):
    def __init__(self, portsTreePath, packagesPath, options):
        ...
        if reportURI == None:
            ...
            self.reporter = ReporterJson(
                reportFile, "master", Configuration.getTargetArchitecture()
            )
            if not self.reporter.connected():
                sysExit("unable to setup json reporting engine")
        elif reportURI.startswith("mongodb://"):
            self.reporter = ReporterMongo(
                reportURI, "master", Configuration.getTargetArchitecture()
            )
            if not self.reporter.connected():
        ...
        if self.localBuilders == 0:
            for fileName in os.listdir(self.builderBaseDir):
                ...
                try:
                    ...
                    self.logger.error(
                        "failed to add builder from config "
                        + configFilePath
                        + ":"
                        + str(exception)
                    )
            ...
            for i in range(0, self.localBuilders):
                ...
                try:
                    builder = LocalBuilder(
                        str(i), packagesPath, self.buildOutputBaseDir, options
                    )
                except Exception as exception:
    ...
    def schedule(self, port, requiredPackageIDs, presentDependencyPackages):
        ...
        for package in port.packages:
            ...
            self.addSkipped(
                port,
                "some packages already exist at "
                + self.packagesPath
    ...
    def _runBuild(self, scheduledBuild):
        while True:
            ...
            threading.Thread(
                ...
                (builder, scheduledBuild, buildNumber),
            ).start()
            break
    ...
    def _buildThread(self, builder, scheduledBuild, buildNumber):
        ...
        if not buildSuccess and reschedule:
            ...
            with open(
                os.path.join(self.buildRecordsDir, str(buildNumber) + ".json"), "w"
            ) as outputFile:
    ...
    def status(self):
        return {
            ...
            "builders": {
                "active": [
                    builder.status
                    for builder in self.activeBuilders
    ...
    def summary(self):
        ...
        if self.impulseIndex >= len(self.impulseData):
            self.impulseIndex = 0
        impulseTime = (
            (self.impulseData[self.impulseIndex]["time"])
            if self.impulseData[self.impulseIndex]
            else None
        )
        impulsePkgCount = (
            (self.impulseData[self.impulseIndex]["pkgCount"])
            if self.impulseData[self.impulseIndex]
            else None
        )
        now = time.time()
        ...
        return {
            ...
            "pkg_hour": (
                int(pkgCount * 3600 / (now - self.startTime))
                if self.startTime
                else None
            ),
            "impulse": (
                int((pkgCount - impulsePkgCount) * 3600 / (now - impulseTime))
                if impulsePkgCount
                else None
            ),
        }
