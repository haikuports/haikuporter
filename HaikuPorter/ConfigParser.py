...
class ConfigParser(object):
    def __init__(self, filename, attributes, shellVariables):
        ...
        wrapperScript = (
            getShellVariableSetters(shellVariables) + configFileEvaluatorScript
        )
        try:
            output = check_output(["bash", "-c", wrapperScript], env=shellEnv).decode(
                "utf-8"
            )
        ...
        for line in lines:
            ...
            if key in attributes:
                ...
                while subKeys:
                    ...
                    if not isPhaseKey:
                        ...
                        warn(
                            "Key %s in file %s is unsupported, ignoring it"
                            % (key, filename)
                        )
                    continue
            ...
            if attrType == bytes:
                ...
                if "any" in entries[key] and len(entries[key]) > 1:
                    sysExit(
                        "%s specifies both 'any' and other architectures" % (filename)
                    )
                if "source" in entries[key] and len(entries[key]) > 1:
                    sysExit(
                        "%s specifies both 'source' and other architectures"
                        % (filename)
                    )
            elif attrType == YesNo:
                valueString = valueString.lower()
                if valueString not in YesNo.getAllowedValues():
                    sysExit(
                        "Value for %s should be 'yes' or 'no' in %s" % (key, filename)
                    )
