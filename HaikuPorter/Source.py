...
class Source(object):
    def __init__(
        ...
        if downloadMirror:
            ...
            self.uris.append(
                downloadMirror + "/" + recipeDirName + "/" + uriFileName + uriExtension
            )
                ...
                for uri in self.uris:
                    ...
                    if baseUri == storedBaseUri:
                        ...
                        if rev != storedRev:
                            ...
                            info(
                                "Skipping download of source for "
                                + self.fetchTargetName
                            )
                        break
                else:
                    warn(
                        "Stored SOURCE_URI is no longer in recipe, automatic "
                        "repository update won't work"
                    )
                    self.sourceFetcher = createSourceFetcher(
        ...
        for uri in self.uris:
            try:
                ...
                if uri != self.uris[-1]:
                    warn(
                        (
                            "Unable to fetch source from %s (error: %s), "
    ...
    def unpack(self, port):
        ...
        self.sourceFetcher.unpack(
            self.sourceBaseDir, self.sourceSubDir, self.sourceExportSubdir
        )
    ...
    def isFromRiggedSourcePackage(self):
        ...
        return (
            self.uris[0].lower().startswith("pkg:")
            ...
            for additionalFile in self.additionalFiles:
                if os.path.isdir(additionalFile):
                    ...
                    for fileName in files:
                        if (
                            os.path.commonprefix([additionalFile, fileName])
                            == additionalFile
                        ):
                            return True
    ...
    def reset(self):
        ...
        output = check_output(
            ["git", "reset", "--hard", "ORIGIN"], cwd=self.sourceDir
        ).decode("utf-8")
        info(output)
        output = check_output(["git", "clean", "-f", "-d"], cwd=self.sourceDir).decode(
            "utf-8"
        )
        info(output)
    ...
    def commitPatchPhase(self):
        ...
        changes = check_output(
            ["git", "status", "--porcelain"], cwd=self.sourceDir
        ).decode("utf-8")
        if not changes:
        ...
        output = check_output(
            ["git", "commit", "-a", "-q", "-m", "patch function"],
            cwd=self.sourceDir,
            env=self.gitEnv,
        ).decode("utf-8")
        info(output)
        output = check_output(
            ["git", "tag", "-f", "PATCH_FUNCTION", "HEAD"], cwd=self.sourceDir
        ).decode("utf-8")
        info(output)
    ...
    def extractPatchset(self, patchSetFilePath, archPatchSetFilePath):
        ...
        if not os.path.exists(self.sourceDir):
            sysExit(
                "Can't extract patchset for "
                + self.sourceDir
                + " as the source directory doesn't exist yet"
            )

        print(
            "Extracting patchset for "
            + self.fetchTargetName
            + " to "
            + patchSetFilePath
        )
        ...
        if needToRebase:
            ...
            check_call(
                ["git", "rebase", "-q", "PATCH_FUNCTION", "haikuport"],
                cwd=self.sourceDir,
                env=self.gitEnv,
            )
        ...
        if os.path.exists(archPatchSetFilePath):
            warn(
                "arch-specific patchset file %s requires manual update"
    ...
    def exportSources(self, targetDir, rigged):
        ...
        if rigged:
            ...
            check_call(
                "tar c --exclude=.git . | tar x -C %s" % targetDir,
                cwd=self.sourceDir,
                shell=True,
            )
        else:
            ...
            self.sourceFetcher.unpack(
                targetDir, self.sourceSubDir, self.sourceExportSubdir
            )
            if self.sourceSubDir:
    ...
    def _initImplicitGitRepo(self):
        ...
        ensureCommandIsAvailable("git")
        info(
            check_output(["git", "init", "-b", "main"], cwd=self.sourceDir).decode(
                "utf-8"
            )
        )
        info(
            check_output(["git", "config", "gc.auto", "0"], cwd=self.sourceDir).decode(
                "utf-8"
            )
        )
        ...
        info(
            check_output(
                ["git", "symbolic-ref", "HEAD", "refs/heads/haikuport"],
                cwd=self.sourceDir,
            ).decode("utf-8")
        )
        info(
