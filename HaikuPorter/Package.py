...
class Package(object):
    def __init__(
        self, packageType, name, port, recipeKeys, policy, isRiggedSourcePackage=False
    ):
        ...
        self.targetMachineTripleAsName = port.shellVariables.get(
    ...
    def isBuildableOnSecondaryArchitecture(
        self, architecture, secondaryArchitecture, forceAllowUnstable=False
    ):
        status = self.getStatusOnSecondaryArchitecture(
    ...
    def makeHpkg(self, requiresUpdater):
        ...
        if requiresUpdater and self.type != PackageType.SOURCE:
            ...
            self.recipeKeys["UPDATED_REQUIRES"] = requiresUpdater.updateRequiresList(
                requiresList
            )
    ...
    def createBuildPackage(self):
        ...
        buildPackageInfo = (
            self.buildPackageDir + "/" + self.revisionedName + "-build.PackageInfo"
        )
        self._generatePackageInfo(
            buildPackageInfo,
        ...
        try:
            ...
            raise Exception(
                "failure creating the build package: "
    ...
    def removeBuildPackage(self):
        ...
        if self.activeBuildPackage:
            buildPlatform.deactivateBuildPackage(
                self.workDir, self.activeBuildPackage, self.revisionedName
            )
            self.activeBuildPackage = None
    ...
    def _generatePackageInfo(
        ...
        with codecs.open(packageInfoPath, "w", "utf-8") as infoFile:
            ...
            self._writePackageInfoListQuotePaths(
                infoFile, self.recipeKeys["HOMEPAGE"], "urls"
            )
            ...
            for index in sorted(
                list(self.recipeKeys["SOURCE_URI"].keys()),
