...
class ChrootSetup(object):
    ...
    def __enter__(self):
        ...
        check_output(
            ["bash", "-c", setupChrootScript], env=shellEnv, cwd=self.path
        ).decode("utf-8")
        return self
...
class Port(object):
    ...
    def __init__(
        ...
        if secondaryArchitecture:
            self.workDir = (
                outputDir + "/work-" + secondaryArchitecture + "-" + self.version
            )
        ...
        self.recipeFilePath = (
            self.baseDir + "/" + self.baseName + "-" + self.version + ".recipe"
        )
    ...
    def validateRecipeFile(self, showWarnings=False):
        ...
        recipeConfig = ConfigParser(
            self.preparedRecipeFile, recipeAttributes, self.shellVariables
        )
        ...
        if not self.isMetaPort:
            ...
            for fileExtension in ["diff", "patch", "patchset"]:
                ...
                for suffix in suffixes:
                    ...
                    if (
                        os.path.exists(self.patchesDir + "/" + patchFileName)
                        and patchFileName not in allPatches
                    ):
                        sysExit(
                            "Patch file %s is not referenced in "
    ...
    def _validateSUMMARY(self, key, entries, showWarnings):
        ...
        if "\n" in entries[key]:
            sysExit(
                "%s must be a single line of text (%s)." % (key, self.recipeFilePath)
            )
        if entries[key].lower().startswith(self.name):
            sysExit(
                "%s cannot start with the name of the port (%s)."
                % (key, self.recipeFilePath)
            )
        if entries[key][0].islower():
            sysExit(
                "%s must start with a capital letter (%s)." % (key, self.recipeFilePath)
            )
    ...
    def _validateDESCRIPTION(self, key, entries, showWarnings):
        ...
        if "SUMMARY" in entries and "".join(entries[key]) == entries["SUMMARY"]:
            sysExit(
                "%s cannot be the same as SUMMARY (in %s)" % (key, self.recipeFilePath)
            )
    ...
    def _validateLICENSE(self, key, entries, showWarnings):
        ...
        if key in entries and entries[key]:
            ...
            for item in recipeLicense:
                ...
                if (
                    item not in fileList
                    and self.licensesDir
    ...
    def _validateCOPYRIGHT(self, key, entries, showWarnings):
        ...
        if key not in entries or not entries[key]:
            ...
            sysExit(
                "%s must not contain e-mail addresses (in %s)"
                % (key, self.recipeFilePath)
            )
        else:
            ...
            if "copyright" in lowerc or "(c)" in lowerc or "©" in lowerc:
                sysExit(
                    '%s must not contain "COPYRIGHT", "(C)", or © (in %s)'
                    % (key, self.recipeFilePath)
                )
    ...
    def checkPortReleases(self):
        ...
        checker = createReleaseChecker(
            self.recipeKeys["SOURCE_URI"]["1"][0], self.version
        )
        if checker:
            newer = checker.check()
            if newer:
                print(
                    "NEWER RELEASE for %s: %s -> %s" % (self.name, self.version, newer)
                )
    ...
    def printDescription(self):
        ...
        for package in self.allPackages:
            ...
            print(
                "STATUS: %s" % package.getStatusOnArchitecture(self.targetArchitecture)
            )
    ...
    def resolveDependencies(
        self, repositories, forTestPhase, presentDependencyPackages=None
    ):
    ...
    def whyIsPortRequired(self, packagesPath, requiredPort):
        ...
        symlinkGlob(
            os.path.join(self._repositoryDir, "*.DependencyInfo"), workRepositoryPath
        )
        ...
        warn(
            "port %s doesn't seem to be required by %s"
    ...
    def extractPatchset(self):
        ...
        for s, source in enumerate(self.sources, 1):
            if s == 1:
                ...
                patchSetFileName = (
                    self.name + "-" + self.version + "-source" + str(s) + ".patchset"
                )
                archPatchSetFileName = (
                    self.name
                    + "-"
    ...
    def build(self, packagesPath, makePackages, hpkgStoragePath):
        ...
        if not (
            getOption("createSourcePackagesForBootstrap")
        ...
        if getOption("createSourcePackagesForBootstrap") or getOption(
            ...
            if not Configuration.isCrossBuildRepository() and not getOption(
                "noSystemPackages"
            ):
        ...
        if buildPlatform.usesChroot():
            ...
            if Configuration.isCrossBuildRepository():
                chrootEnvVars["crossSysrootDir"] = self.shellVariables[
                    "crossSysrootDir"
                ]
            ...
            buildPlatform.setupNonChrootBuildEnvironment(
                self.workDir, self.secondaryArchitecture, allPackages
            )
            try:
                self._executeBuild(makePackages)
            except:
                buildPlatform.cleanNonChrootBuildEnvironment(
                    self.workDir, self.secondaryArchitecture, False
                )
                raise
            buildPlatform.cleanNonChrootBuildEnvironment(
                self.workDir, self.secondaryArchitecture, True
            )
    ...
    def _parseRecipeFile(self, showWarnings, forceAllowUnstable=False):
        ...
        self.recipeKeysByExtension, self.definedPhases = self._validateOrLoadFromCache(
            showWarnings
        )
        self.recipeKeys = {}
        ...
        for index in sorted(
            list(keys["SOURCE_URI"].keys()), key=cmp_to_key(naturalCompare)
        ):
            source = Source(
                self,
                index,
    ...
    def _updateShellVariables(self, forParsing):
        ...
        self.shellVariables["configureDirArgs"] = " ".join(
            "--%s=%s" % (k.lower(), v) for k, v in configureDirs.items()
        )
    ...
    def _recreatePackageDirectories(self):
        ...
        directoriesToCreate = [
            self.packageInfoDir,
            self.packagingBaseDir,
            self.buildPackageDir,
            self.hpkgDir,
        ]
        directoriesToRemove = [
    ...
    def _getNeededPackages(
        self, packagesPath, requiresTypes, description, forTestPhase=False
    ):
        return self._resolveDependencies(
            self.getDependencyInfoFiles(),
            requiresTypes,
            [packagesPath],
            description,
            ignoreBase=forTestPhase,
    ...
    def _getPackagesNeededForScriptlets(self, packagesPath):
        ...
        return self._getNeededPackages(
            packagesPath, ["SCRIPTLET_PREREQUIRES"], "needed packages for scriptlets"
        )
    ...
    def _getPackagesRequiredForBuild(self, packagesPath):
        ...
        return self._getNeededPackages(
            packagesPath, ["BUILD_REQUIRES"], "required packages for build"
        )
    ...
    def _doRecipeAction(self, action, targetDir):
        ...
        wrapperScriptContent = (
            getShellVariableSetters(shellVariables) + recipeActionScript
        )
    ...
    def _openShell(self, params, targetDir, env):
        ...
        if self.logger is None:
            ...
            process = Popen(
                args, cwd=targetDir, env=shellEnv, stdout=PIPE, stderr=STDOUT
            )
    ...
    def _resolveDependencies(
        self, dependencyInfoFiles, requiresTypes, repositories, description, **kwargs
    ):
        """Resolve dependencies of one or more dependency-infos"""

        try:
            return buildPlatform.resolveDependencies(
