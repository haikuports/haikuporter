...
from .Utils import info, sysExit, warn
...
class ReporterMongo(object):
    ...
    def updateBuildrun(self, buildNumber, status):
        ...
        buildrunCollection.update_one(
            {"_id": buildNumber}, {"$set": mdbStatus}, upsert=True
        )
    ...
    def _updateBuilders(self, status):
        ...
        for state in status["builders"].keys():
            for builder in status["builders"][state]:
                ...
                builderCollection.update_one(
                    {"_id": builder["name"]}, {"$set": bldStatus}, upsert=True
                )
