...
class SourceFetcherForBazaar(object):
    ...
    def fetch(self):
        if not Configuration.shallAllowUnsafeSources():
            sysExit(
                "Downloading from unsafe sources is disabled in " + "haikuports.conf!"
            )
    ...
    def updateToRev(self, rev):
        warn(
            "Updating of a Bazaar repository to a specific revision has "
...
class SourceFetcherForCvs(object):
    ...
    def fetch(self):
        ...
        output = check_output(command, shell=True, cwd=baseDir, stderr=STDOUT).decode(
            "utf-8"
        )
        info(output)
    ...
    def updateToRev(self, rev):
        warn(
            "Updating of a CVS repository to a specific revision has "
            "not been implemented yet, sorry"
        )
...
class SourceFetcherForFossil(object):
    ...
    def updateToRev(self, rev):
        warn(
            "Updating of a Fossil repository to a specific revision has "
            "not been implemented yet, sorry"
        )
...
class SourceFetcherForGit(object):
    ...
    def updateToRev(self, rev):
        ...
        try:
            output = check_output(command, shell=True, cwd=self.fetchTarget).decode(
                "utf-8"
            )
            info(output)
        except:
            ...
            branches = (
                check_output(command, shell=True, cwd=self.fetchTarget, stderr=STDOUT)
                .decode("utf-8")
                .splitlines()
            )
    ...
    def unpack(self, sourceBaseDir, sourceSubDir, foldSubDir):
        sourceDir = (
            sourceBaseDir + "/" + sourceSubDir if sourceSubDir else sourceBaseDir
        )
        if foldSubDir:
...
class SourceFetcherForSubversion(object):
    ...
    def updateToRev(self, rev):
        warn(
            "Updating of a Subversion repository to a specific revision has "
            "not been implemented yet, sorry"
        )
