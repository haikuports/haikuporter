...
class RemoteBuilderSSH(object):
    def __init__(
        ...
        logHandler = logging.FileHandler(
            os.path.join(self.builderOutputDir, self.name + ".log"), encoding="utf-8"
        )
    ...
    def _loadConfig(self, configFilePath):
        ...
        if "privateKeyFile" not in self.config["ssh"]:
            raise Exception(
                "missing ssh privateKeyFile config for builder " + self.name
            )
    ...
    def _writeBuilderConfig(self):
        try:
            ...
            with self._openRemoteFile(
                self.config["portstree"]["builderConfig"], "w"
            ) as remoteFile:
        ...
        try:
            ...
            for package in scheduledBuild.port.packages:
                self.buildLogger.info(
                    "download package " + package.hpkgName + " from builder"
                )
    ...
    def _move(self, sourcePath, destPath):
        ...
        (output, channel) = self._remoteCommand(
