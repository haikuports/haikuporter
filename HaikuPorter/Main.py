...
class Main(object):
    ...
    def run(self, args):
        ...
        if (
            not self.options.noSystemPackages
        ...
        if self.options.buildMaster:
            ...
            self.buildMaster = BuildMaster(
                self.treePath, self.packagesPath, self.options
            )
        ...
        if self.options.lint:
            if (
                ...
                sysExit(
                    "LICENSES_DIRECTORY must be set in configuration on "
                    "this build platform!"
                )
            self._createRepositoryIfNeeded(True)
            if not args:
        ...
        if self.options.list or self.options.listPackages:
            ...
            if self.options.list:
                allNames = self.repository.searchPorts(
                    None, self.options.printFilenames
                )
            else:
                allNames = self.repository.searchPackages(
                    None, self.options.printFilenames
                )
        ...
        if self.options.search or self.options.searchPackages:
            if not args:
                sysExit(
                    "You need to specify a search string.\nInvoke '"
                    + sys.argv[0]
            ...
            for arg in args:
                if self.options.search:
                    ...
                    packageNames = self.repository.searchPackages(
                        arg, self.options.printFilenames
                    )
        ...
        if self.options.portsForFiles:
            ...
            files = [
                arg if os.path.isabs(arg) else os.path.join(self.treePath, arg)
                for arg in args
            ]
        ...
        if self.options.portsfile:
            ...
            for port in ports:
                ...
                if portSpec["version"]:
                    portsfileAsRequires.append(
                        portSpec["name"] + " ==" + portSpec["version"]
                    )
                else:
        ...
        if self.options.portsfile:
            ...
            for portId in portsToBuild:
                ...
                if mainPackage and os.path.exists(
                    self.packagesPath + "/" + mainPackage.hpkgName
                ):
                    print(
                        "skipping port %s, since its main package already exists"
                        % portId
                    )
                    continue
            ...
            self.portSpecs = [
                self._splitPortSpecIntoNameVersionAndRevision(port) for port in args
            ]
        ...
        for portSpec in self.portSpecs:
            ...
            if portName not in portVersionsByName:
                ...
                if Configuration.isCrossBuildRepository():
                    nameWithTargetArch = (
                        portName + "_" + self.shellVariables["targetArchitecture"]
                    )
                    ...
                    if not portName:
                        if self.options.buildMaster:
                            self.buildMaster.addSkipped(
                                portSpec["name"], "not found in repository"
                            )
                            continue
            ...
            if portSpec["version"]:
                ...
                if not version:
                    if self.options.buildMaster:
                        self.buildMaster.addSkipped(
                            portSpec["name"],
                            "no version of " + portSpec["name"] + " can be built",
                        )
                        continue
                    else:
        ...
        if self.options.why:
            ...
            if not whySpec["version"]:
                whySpec["version"] = self.repository.getActiveVersionOf(
                    whySpec["name"], False
                )
        ...
        for portSpec in self.portSpecs:
            ...
            if self.options.clean:
                ...
                (self.options.build and portSpec["id"] not in bootstrapPorts)
                or self.options.test
    ...
    def _listDependencies(self, port):
        ...
        presentDependencyPackageNames = [
            os.path.basename(package) for package in presentDependencyPackages
        ]
    ...
    def _validateMainPort(self, port, revision=None):
        ...
        if revision and port.revision != revision:
            sysExit(
                (
                    "Port %s isn't available in revision %s (found revision "
                    + "%s instead)"
                )
    ...
    def _buildMainPort(self, port, testPort):
        ...
        if self.options.buildMaster:
            ...
            try:
                ...
                self.buildMaster.addSkipped(
                    port, "resolving build dependencies failed: {}".format(exception)
                )
                return
        else:
            buildDependencies = port.resolveDependencies(
            ...
            try:
                ...
                if portID not in requiredPortIDs:
                    ...
                    if (
                        ...
                        not requiredPort.sourcePackage
                        or requiredPort.sourcePackageExists(self.packagesPath)
                    ):
                        continue
            ...
            for requiredPort in requiredPortsToBuild:
                if self.options.buildMaster:
                    ...
                    try:
                        ...
                        sysExit(
                            "Dependency of " + port.versionedName + " cannot be built"
                        )
                else:
                    self._buildPort(requiredPort, True, False)

        if self.options.buildMaster:
            self.buildMaster.schedule(
                port, requiredPackageIDs, presentDependencyPackages
            )
    ...
    def _setupForPossiblyObsoletePort(self, port):
        ...
        if port.version != activeVersion:
            ...
            warn(
                "building obsolete port, packages will be put in {}".format(targetPath)
            )
    ...
    def _initGlobalShellVariables(self):
        ...
        if Configuration.isCrossBuildRepository():
            ...
            if not targetHaikuPackage:
                if not buildPlatform.isHaiku:
                    sysExit(
                        "On this platform a haiku cross devel package "
            ...
            if (
                ...
                sysExit(
                    "Native building not supported on this platform (%s)"
                    % buildPlatform.name
                )
        ...
        self.shellVariables = {
            "haikuVersion": (
                "r1~alpha1"
            ),  # just a dummy value for compatibility with old recipes
    ...
    def _checkFormatVersions(self):
        ...
        if os.path.exists(formatVersionsFile):
            ...
            recipeFormatVersionMatch = re.search(
                "^RecipeFormatVersion=(.+?)$", formatVersions, flags=re.MULTILINE
            )
