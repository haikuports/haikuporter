...
class Display:
	def __init__(self, stdscr, builders_used):
		...
		self.zone_actions  = curses.newwin(y - (self.builders_used+4+2), ncols, self.builders_used+4+2, 0)

		self.zone_summary.addstr(0, 0, " Total         Built        Ignored        Load  0.00  Pkg/hour                ", self.c_sumlabel)
		self.zone_summary.addstr(1, 0, "  Left        Failed        Skipped        Swap  0.0%   Impulse	      0:00:00  ", self.c_sumlabel)
		...
		self.zone_builders.addstr(1, 0, " ID  Duration  Build Phase      Port Name                                Lines ", self.c_tableheader)
		...
		self.zone_actions.refresh()

	def updateSummary(self, data):
		self.zone_summary.addstr(0, 7,
str(data['builds']['total']).ljust(4),
			self.c_standard)
		self.zone_summary.addstr(0, 21,
str(data['builds']['complete']).ljust(4),
			self.c_success)
		self.zone_summary.addstr(0, 37,
str(data['builds']['blocked']).ljust(4),
			self.c_blocked)
		self.zone_summary.addstr(0, 64,
str(data['pkg_hour']).ljust(5),
			self.c_standard)
		self.zone_summary.addstr(1, 7, str(data['builds']['scheduled']
			+ data['builds']['active'] +
data['builds']['blocked']).ljust(4),
			self.c_standard)
		self.zone_summary.addstr(1, 21,
str(data['builds']['failed']).ljust(4),
			self.c_failure)
		self.zone_summary.addstr(0, 37,
str(data['builds']['lost']).ljust(4),
			self.c_lost)
		self.zone_summary.addstr(1, 64,
str(data['impulse']).ljust(5),
			self.c_standard)
		self.zone_summary.addstr(1, 70, str(datetime.timedelta(
			seconds=int(data['duration']))) if data['duration'] else '       ',
			self.c_duration)
		self.zone_summary.refresh()
	...
	def updateBuilders(self, data):
		...
		for i in range(0, self.builders_used):
			...
			self.zone_builders.addstr(3+i, 5, str(datetime.timedelta(
				seconds=int(currentBuild['duration']))
				)[:8] if currentBuild['duration'] else '        ',
				self.c_standard)
			self.zone_builders.addstr(3+i, 15, currentBuild['phase'].ljust(12)[:12],
				self.c_bldphase)
			self.zone_builders.addstr(3+i, 32, portName.ljust(38)[:38],
				self.c_portname)
			self.zone_builders.addstr(3+i, 71, str(currentBuild['lines']).rjust(7)[:7],
				self.c_standard)
